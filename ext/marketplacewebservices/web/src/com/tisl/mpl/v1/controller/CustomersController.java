...
import com.tisl.mpl.conv.HttpRequestPaymentInfoPopulator;
import com.tisl.mpl.populator.HttpRequestAddressDataPopulator;
...
import com.tisl.mpl.user.data.LoginChangeResponse;
import com.tisl.mpl.user.data.PasswordRestoreResponse;
...
public class CustomersController extends BaseController
{
	 ...
	 * @ResponseBody
	 * 
	 * @ResponseStatus(value = HttpStatus.CREATED) public void registerUser(@RequestParam final String login,
	 * 
	 * @RequestParam final String password,
	 * 
	 * @RequestParam(required = false) final String titleCode, @RequestParam final String firstName,
	 * 
	 * @RequestParam final String lastName) throws DuplicateUidException, RequestParameterException { if
	 * (LOG.isDebugEnabled()) { LOG.debug("registerUser: login=" + sanitize(login)); }
	 * 
	 * if (!EmailValidator.getInstance().isValid(login)) { throw new RequestParameterException("Login [" +
	 * sanitize(login) + "] is not a valid e-mail address!", RequestParameterException.INVALID, "login"); }
	 * 
	 * final RegisterData registration = new RegisterData(); registration.setFirstName(firstName);
	 * registration.setLastName(lastName); registration.setLogin(login); registration.setPassword(password);
	 * registration.setTitleCode(titleCode); customerFacade.register(registration); }
	 */
	...
	@ResponseBody
	public AddressData createAddress(final HttpServletRequest request) throws DuplicateUidException, WebserviceValidationException
	{
