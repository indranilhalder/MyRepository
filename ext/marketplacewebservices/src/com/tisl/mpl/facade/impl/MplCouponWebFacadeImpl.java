...
public class MplCouponWebFacadeImpl implements MplCouponWebFacade
{
			...
			JaloInvalidParameterException, JaloSecurityException, EtailBusinessExceptions, EtailNonBusinessExceptions
	{
		...
		catch (final VoucherOperationException e)
		{
			...
			else if (e.getMessage().contains(MarketplacecouponConstants.SELLER_RESTRICTION))
			{
				throw new EtailNonBusinessExceptions(e, "B95131");
			}
			else if (e.getMessage().contains("order"))
			{
				throw new EtailNonBusinessExceptions(e, "B95141");
			}
			...
			JaloSecurityException, JaloPriceFactoryException, EtailBusinessExceptions
	{
		...
		try
		{
			...
			if (null != cartModel)
			{
				...
				// EGV Changes END
				if (StringUtils.isNotEmpty(cartCouponCode)
						&& cartModel.getTotalPrice().doubleValue() > cartModel.getPayableWalletAmount().doubleValue())
				{
					try
					{
						cartModel.setCheckForBankVoucher("true");
			...
			else if (null != orderModel)
			{
				...
				// EGV Changes END
				if (StringUtils.isNotEmpty(cartCouponCode)
						&& orderModel.getTotalPrice().doubleValue() > orderModel.getPayableWalletAmount().doubleValue())
				{
					try
					{
						orderModel.setCheckForBankVoucher("true");
