 ...
 */
public class MplDefaultCartLoaderStrategy implements CartLoaderStrategy
{
	...
	protected void applyCurrencyToCartAndRecalculateIfNeeded()
	{
		...
		if (!cart.getCurrency().equals(currentCurrency))
		{
			...
			//PROD Stabilization fix
			/*
			 * try { commerceCartService.recalculateCart(cart); } catch (final CalculationException e) { throw new
			 * CartException("Couldn't recalculate cart" + e.getMessage(), CartException.CANNOT_RECALCULATE, e); }
			 */
		}
	}
