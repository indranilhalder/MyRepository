...
public class MplEgvWalletServiceImpl implements MplEgvWalletService
{
	 ...
	 *
	 * @see
	 * com.tisl.mpl.service.MplEgvWalletService#createWalletContainer(de.hybris.platform.core.model.user.CustomerModel)
	 */
	@Override
	public QCCustomerRegisterResponse createWalletContainer(final CustomerModel currentCustomer)
	{
	 ...
	 * (non-Javadoc)
	 *
	 * @see com.tisl.mpl.service.MplEgvWalletService#verifyOtpAndCreateWallet(de.hybris.platform.core.model.user.
	 * CustomerModel, java.lang.String)
	 */
	@Override
	public EgvWalletCreateResponceWsDTO verifyOtpAndCreateWallet(final CustomerModel currentCustomer, final String otp,
			final String firstName, final String lastName, final String mobileNumber)
	{
		final EgvWalletCreateResponceWsDTO responce = new EgvWalletCreateResponceWsDTO();
		if (null != otp && !otp.isEmpty())
		{
			...
			final OTPResponseData response = mplWalletFacade.validateOTP(currentCustomer.getUid(), otp);
			...
			if (response.getOTPValid().booleanValue())
			{
				if (customerWalletCreated)
				{
					...
					final CustomerWalletDetailResponse customerWalletDetailData = mplWalletFacade
							.getCustomerWallet(currentCustomer.getCustomerWalletDetail().getWalletId());
					final UserCliqCashWsDto userCliqCashData = getCustomerWalletAmount(customerWalletDetailData);
			...
			else
			{
				if (null != response.getInvalidErrorMessage())
				{
					...
					else if (response.getInvalidErrorMessage()
							.equalsIgnoreCase(MarketplacecommerceservicesConstants.INVALID_WALLET_OTP))
					{
	 ...
	 */
	@Override
	public UserCliqCashWsDto getCustomerWalletAmount(final CustomerWalletDetailResponse customerWalletDetailData)
	{
		try
		{
			final UserCliqCashWsDto userCliqCashWsDto = new UserCliqCashWsDto();
	 ...
	 */
	@Override
	public UserCliqCashWsDto getUserCliqCashDetails(final CustomerModel currentCustomer)
	{
		...
		if (null != currentCustomer)
		{
			...
			{
				final WalletCreateData walletCreateData = mplWalletFacade.getWalletCreateData(currentCustomer);
				responce.setStatus(MarketplacecommerceservicesConstants.SUCCESS_FLAG);
		...
		try
		{
			...
			if (null != walletAmount && walletAmount.doubleValue() > 0.0D)
			{
				...
				else
				{
					...
					final Double amount = Double.valueOf(juspayTotalAmt);
					final BigDecimal bigDecimal = new BigDecimal(amount.doubleValue());
					final String decimalFormat = "0.00";
					final DecimalFormat df = new DecimalFormat(decimalFormat);
					final String totalPayableAmount = df.format(bigDecimal);
	 ...
	 */
	@Override
	public void useCliqCash(final AbstractOrderModel order)
	{
		try
		{
			final double cartTotal = order.getTotalPrice().doubleValue();
			final double totalWalletAmount = order.getTotalWalletAmount().doubleValue();
			...
			if (totalWalletAmount > 0.0D)
			{
				...
				final double payableJuspayAmount = cartTotal - payableWalletAmount;
	 ...
	 */
	@Override
	public boolean generateOtpForUpdateWallet(final String mobileNumber, final CustomerModel customer)
	{
	 ...
	 * (non-Javadoc)
	 *
	 * @see com.tisl.mpl.service.MplEgvWalletService#setTotalPrice(com.tisl.mpl.wsdto.ApplyCliqCashWsDto,
	 * de.hybris.platform.core.model.order.AbstractOrderModel)
	 */
	@Override
	public ApplyCliqCashWsDto setTotalPrice(final ApplyCliqCashWsDto applyCliqCashWsDto, final AbstractOrderModel cartModel)
	{
	 ...
	 *
	 * @see
	 * com.tisl.mpl.service.MplEgvWalletService#checkWalletMobileNumber(com.tisl.mpl.wsdto.EgvWalletCreateRequestWsDTO,
	 * boolean)
	 */
	@Override
	public EgvCheckMobileNumberWsDto checkWalletMobileNumber(final EgvWalletCreateRequestWsDTO request,
			final boolean isUpdateProfile)
	{
		final EgvCheckMobileNumberWsDto responce = new EgvCheckMobileNumberWsDto();
		...
		if (null != request)
		{
			...
			if (isUpdateProfile && isWalletCreated)
			{
				if (null == request.getMobileNumber()
						|| (StringUtils.length(request.getMobileNumber()) != MarketplacecommerceservicesConstants.MOBLENGTH
				...
				final boolean isOtpGenerated = this.generateOtpForUpdateWallet(request.getMobileNumber(), customer);
				if (isOtpGenerated)
				{
					responce.setStatus(MarketplacecommerceservicesConstants.SUCCESS_FLAG);
					responce.setOtpExpiryTime(
							configurationService.getConfiguration().getString(MarketplacecommerceservicesConstants.TIMEFOROTP));
					return responce;
				}
				else
				{
	 ...
	 * @param request
	 */
	private void validateRequest(final EgvWalletCreateRequestWsDTO request)
	{
		if (null == request)
		{
			throw new EtailBusinessExceptions(MarketplacecommerceservicesConstants.B5015);
	 ...
	 * (non-Javadoc)
	 *
	 * @see com.tisl.mpl.service.MplEgvWalletService#setTotalPrice(com.tisl.mpl.wsdto.ApplyCartCouponsDTO,
	 * de.hybris.platform.core.model.order.CartModel)
	 */
	@Override
	public ApplyCartCouponsDTO setTotalPrice(final ApplyCartCouponsDTO applycouponDto, final AbstractOrderModel cartModel)
	{
		final double payableWalletAmount = cartModel.getPayableWalletAmount().doubleValue();
		double bankCouponDiscount = 0.0D;
		double userCouponDiscount = 0.0D;
	 ...
	 * (non-Javadoc)
	 *
	 * @see com.tisl.mpl.service.MplEgvWalletService#updateWallet(de.hybris.platform.core.model.user.CustomerModel,
	 * java.lang.String, com.tisl.mpl.facades.product.data.MplCustomerProfileData)
	 */
	@Override
	public boolean updateWallet(final CustomerModel customer, final String otp, final MplCustomerProfileData customerToSave)
	{
		...
		try
		{
			...
			{
				final OTPResponseData response = mplWalletFacade.validateOTP(customer.getUid(), otp);
	 ...
	 *
	 */
	public boolean updateCustomerWallet(final MplCustomerProfileData customerToSave, final CustomerModel customer)
	{

		final QCCustomerRegisterRequest registerCustomerRequest = new QCCustomerRegisterRequest();
		...
		final String walletId = customer.getCustomerWalletDetail().getWalletId();
		final CustomerWalletDetailResponse responce = mplWalletFacade.updateCustomerWallet(registerCustomerRequest, walletId,
	 ...
	 *
	 * @see
	 * com.tisl.mpl.service.MplEgvWalletService#getEgvProductDetails(de.hybris.platform.core.model.user.CustomerModel)
	 */
	@Override
