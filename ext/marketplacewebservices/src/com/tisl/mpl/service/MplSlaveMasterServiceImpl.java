		...
		if (!list.isEmpty())
		{
			for (final SlaveInfoDTO slaveInfoDto : list)
			{
				...
				if (posModel != null)
				{
					...
					{
						posModel.setType(PointOfServiceTypeEnum.POS.STORE);
					}
					...
					try
					{
						...
						{
							if (!slaveInfoDto.getOpeningTime().equalsIgnoreCase(df.format(posModel.getOpeningTime())))
							{
								posModel.setOpeningTime(df.parse(slaveInfoDto.getOpeningTime()));
							}
						}
						if (StringUtils.isNotEmpty(slaveInfoDto.getClosingTime()))
						{
							if (!slaveInfoDto.getClosingTime().equalsIgnoreCase(df.format(posModel.getClosingTime())))
							{
								posModel.setClosingTime(df.parse(slaveInfoDto.getClosingTime()));
							}
						}
					}
					catch (final ParseException e1)
					{
						LOG.debug("Exception while setting date for POS: " + e1.getMessage());
