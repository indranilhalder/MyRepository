...
public class UnCollectedOrderToInitiateRefundEventListener extends AbstractEventListener<UnCollectedOrderToInitiateRefundEvent>
{
	...
	@Override
	protected void onEvent(UnCollectedOrderToInitiateRefundEvent unCollectedOrderToInitiateRefundEvent)
	{
	try{
		...
		if (unCollectedOrderToInitiateRefundEvent.getShipmentNewStatus().equals(ConsignmentStatus.CANCELLATION_INITIATED))
		{
      		 for (final AbstractOrderEntryModel orderEntryModel : unCollectedOrderToInitiateRefundEvent.getOrderModel().getEntries())
      		  {
					if(unCollectedOrderToInitiateRefundEvent.getShipment().getShipmentId().equals(orderEntryModel.getTransactionID()) && unCollectedOrderToInitiateRefundEvent.getConsignmentModel().getCode().equals(orderEntryModel.getTransactionID())){
						
						final String trackOrderUrl = unCollectedOrderToInitiateRefundEvent.getConfigurationService().getConfiguration().getString(MarketplacecommerceservicesConstants.SMS_ORDER_TRACK_URL) + unCollectedOrderToInitiateRefundEvent.getOrderModel().getCode();
						...
						{
							LOG.error("Exception during Refund Email Trigger for Un-Collected Orders >> "+ unCollectedOrderToInitiateRefundEvent.getConsignmentModel().getCode() +" -- " + e.getMessage());
						}
	 ...
	 * @return
	 */
	private boolean checkRefundStatus(UnCollectedOrderToInitiateRefundEvent unCollectedOrderToInitiateRefundEvent,
			final AbstractOrderEntryModel orderEntryModel)
	{
		return customOmsCancelAdapter.initiateCancellation(MarketplaceomsordersConstants.TICKET_TYPE_CODE,
				orderEntryModel.getTransactionID(), unCollectedOrderToInitiateRefundEvent.getOrderModel(), MarketplaceomsordersConstants.REASON_CODE,
				unCollectedOrderToInitiateRefundEvent.getConsignmentModel());
	}
