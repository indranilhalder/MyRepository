...
<c:choose>
	<c:when test="${isCart eq true}">
		...
		<c:set var="deliveryAddressLine2"
			value="${fn:trim(deliveryAddress.line2)}" />
		<c:set var="deliveryAddressLine3"
			value="${fn:trim(deliveryAddress.line3)}" />
		...
		<c:if test="${hasShippedItems}">
			...
			<c:choose>
				<c:when test="${showDeliveryAddress and not empty deliveryAddress}">
					...
					<address>
						<span> <b>${fn:escapeXml(deliveryAddress.title)}${fn:escapeXml(deliveryAddress.firstName)}&nbsp;${fn:escapeXml(deliveryAddress.lastName)}</b>
							<c:if test="${ not empty deliveryAddress.line1 }">
						${fn:escapeXml(deliveryAddress.line1)},&nbsp;
					</c:if> <c:if test="${ not empty deliveryAddressLine2 }">
					
						${fn:escapeXml(deliveryAddress.line2)},
					</c:if> <c:if test="${ not empty deliveryAddressLine3 }">
					
						${fn:escapeXml(deliveryAddress.line3)},
					</c:if> <c:if
								test="${ not empty deliveryAddress.landmark && deliveryAddress.landmark != 'NA'}">
						${fn:escapeXml(deliveryAddress.landmark)},
					</c:if> <br> <c:if test="${not empty deliveryAddress.town }">
						${fn:escapeXml(deliveryAddress.town)},&nbsp;
					</c:if> <c:if test="${ not empty deliveryAddress.region.name }">
						${fn:escapeXml(deliveryAddress.region.name)},&nbsp;
					</c:if> <br> <c:if test="${ not empty deliveryAddress.state }">
						${fn:escapeXml(deliveryAddress.state)},&nbsp;
					</c:if> <c:if test="${ not empty deliveryAddress.postalCode }">
						${fn:escapeXml(deliveryAddress.postalCode)}&nbsp;
					</c:if> <c:if test="${ not empty deliveryAddress.country.name }">
						${fn:escapeXml(deliveryAddress.country.isocode)}
					</c:if> <br> <c:if test="${ not empty deliveryAddress.phone }">
								<spring:theme code="checkout.phone.no" text="+91" />&nbsp;${fn:escapeXml(deliveryAddress.phone)}&nbsp;
					</c:if> <br> <!-- For TISST-11525 --> <c:if
								test="${ not empty deliveryAddress.addressType }">
								<spring:theme code="checkout.pickup.addressType"
									text="Address Type:" />&nbsp;
					  <c:if test="${deliveryAddress.addressType eq 'Home'}">
							...
							</c:if> <!-- For TISST-11525 Ends -->

						</span>
					</address>
				</c:when>
				<c:otherwise>
					<div class="alternatetitle">
		...
		<hr>
		<ul class="prd-info">
			<li class="items-shipped-count item-adds"><c:if
					test="${ not empty cartData.totalUnitCount }">
					...
					<c:if test="${hasShippedItems}">
						<c:if test="${cartData.totalUnitCount > 0}">${cartData.deliveryItemsQuantity}&nbsp;<spring:theme
								code="basket.page.totalQtyForAddress.items" />
						</c:if>
						<%-- 	<c:if test="${cartData.totalUnitCount <= 1}">${cartData.totalUnitCount}&nbsp;<spring:theme code="basket.page.totalQtyForAddress.item" /></c:if> > --%>
					</c:if>
				</c:if></li>
					...
					<li class="item">
						...
						<div class="product">
							...
							<div class="prd-rate">
								...
								<p id="${entry.selectedUssid}_${entry.giveAway}_productPriceId"
                  ...
                  <c:out value="${entry.mplDeliveryMode.description}"></c:out>
                    ...
                    <p class="delivery-est">Delivered in 3-5 business days</p>
								...
								<div class="variants">
									...
									<c:if
										test="${ycommerce:doesPotentialPromotionExistForOrderEntry(cartData, entry.entryNumber) && showPotentialPromotions && (entry.isBOGOapplied || entry.giveAway)}">
										<ul>
									...
									<c:if
										test="${ycommerce:doesAppliedPromotionExistForOrderEntry(cartData, entry.entryNumber) && (entry.isBOGOapplied || entry.giveAway) }">
										<ul>
						...
						</div> <%-- <div class="price"><format:price priceData="${entry.basePrice}" displayFreeForZero="true"/></div>
			<div class="details">
			...
			</div> --%> <!--  <div class="stock-status">Item In Stock</div> -->

					</li>
					<li>
						<div class="method item-mthd">
							<h3>
								<spring:theme code="checkout.multi.shipmentMethod" />
							</h3>
							<p class="delivery-method-description">
								...
								<c:if test="${entry.currDelCharge.value.unscaledValue() != 0}">
									<c:out value="${entry.currDelCharge.formattedValue}"></c:out>
								</c:if>
							</p>
							<p
								class="delivery-method-description delivery-method-description-time">
							...
							<c:choose>
								<c:when
									...
									<p
										class="delivery-method-description delivery-method-description-time">
										Delivered ON ${entry.selectedDeliverySlotDate} Between
										${entry.timeSlotFrom} TO ${entry.timeSlotTo}</p>
								</c:when>
								<c:otherwise>
									<p
										class="delivery-method-description delivery-method-description-time">
										Your Order Will Be Delivered ${entry.eddDateBetWeen}</p>
	...
	<c:otherwise>
		<c:set var="hasShippedItems"
			value="${orderData.deliveryItemsQuantity > 0}" />
		...
		<ul>
			<li class="items-shipped-count"><c:if
					test="${ not empty orderData.totalUnitCount }">
					<c:if test="${not hasShippedItems}">
						<c:if test="${orderData.totalUnitCount > 0}">${orderData.totalUnitCount}&nbsp; ITEMS PICKUP FROM BELOW STORE</c:if>
					</c:if>
					<c:if test="${hasShippedItems}">
						<c:if test="${orderData.totalUnitCount > 0}">${orderData.deliveryItemsQuantity}&nbsp;<spring:theme
								code="basket.page.totalQtyForAddress.items" />
						</c:if>
					</c:if>
				</c:if></li>
					...
					<li class="item">
						...
						<div class="product">
							...
							<div class="method">
								...
								<p class="delivery-method-description">
									<c:out value="${entry.mplDeliveryMode.name}"></c:out>
									&nbsp;-&nbsp;
									<c:if test="${entry.currDelCharge.value.unscaledValue() == 0}">
										<c:out value="FREE"></c:out>
									</c:if>
                    ...
                    <p class="delivery-est">Delivered in 3-5 business days</p>
						...
						<ul>
							<c:forEach items="${orderData.potentialProductPromotions}" var="promotion">
								...
								<c:if
									test="${ycommerce:doesAppliedPromoExistForOrderEntry(orderData, entry.entryNumber) && (entry.isBOGOapplied || entry.giveAway) }">
									<ul>
										<c:forEach items="${orderData.appliedProductPromotions}"
											var="promotion">
											<c:set var="displayed" value="false" />
											<c:forEach items="${promotion.consumedEntries}"
												var="consumedEntry">
