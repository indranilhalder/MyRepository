...
<%@ attribute name="defaultPincode" required="true"
...
<c:if test="${hasShippedItems}">
	...
	<div class="checkout-shipping-items">
		<div class="checkout-headers">
			<h1 class="title-name">
				<spring:theme
					code="checkout.multi.deliveryMethod.chooseDeliveryOption"></spring:theme>
				</br>
			</h1>
			<p class="desk-view">
				<spring:theme
					code="checkout.multi.deliveryMethod.chooseDeliveryOption.showPincode"
					text="Showing delivery options for pincode"></spring:theme>
				<span>${defaultPinCode}</span>
			</p>
		</div>
		...
		<ul id="deliveryradioul"
			class="checkout-table product-block mybag-items checkout-items">
			<li class="header">
				<ul class="headline mybag-item-head">

					<li id="header2" class="Product"><spring:theme
							code="text.product.delivery.product" /></li>
					<li id="header2" class="Price"><spring:theme
							code="text.product.delivery.price" /></li>
					<li id="header2" class="qty"><spring:theme
							code="text.product.delivery.quantity" /></li>
					<li class="delivery dev" id="header4"><spring:theme
							code="text.product.delivery.deliveryoption" /></li>
				</ul>
			</li>
					...
					<ul>
						<li>
							<div>
								...
								<div class="details product">
									...
									<c:if
										test="${fn:toUpperCase(entry.product.rootCategory) != 'ELECTRONICS'}">
									...
									<c:if test="${not empty sellerInfoMap}">
										<c:forEach items="${sellerInfoMap}" var="sellerInfoMap">
											<c:if test="${sellerInfoMap.key == entry.entryNumber}">
												<c:forEach items="${sellerInfoMap.value}"
													var="sellerInfoMapValue">
													<div>
														<span><spring:theme code="text.seller.name" /> </span>${sellerInfoMapValue}</div>
												</c:forEach>

												<div class="size">
													<spring:theme code="text.size" />${entry.product.size}</div>
												<div class="colour">
													<spring:theme code="text.colour" />${entry.product.colour}</div>
											</c:if>
										</c:forEach>
									</c:if>
							...
							</div> <%-- <div class="item-price delivery-price">
											<ycommerce:testId code="cart_totalProductPrice_label">
												<format:price priceData="${entry.totalPrice}"
													displayFreeForZero="true" />
											</ycommerce:testId>
						...
						<li class="quantity">
							<div>
								<ycommerce:testId code="cart_product_colour">
									<div class="colour">
										<spring:theme code="text.qty" />${entry.quantity}</div>
								</ycommerce:testId>
							</div>
						<li class="delivery">
							<ul>
								<c:if test="${not empty deliveryModeData}">
									<c:forEach items="${deliveryModeData}"
										var="deliveryModeDataMap">

										<c:if test="${deliveryModeDataMap.key == entry.entryNumber}">

											<c:set var='count' value='1' />
											<c:set var="delModes"
												value="${fn:length(deliveryModeDataMap.value)}" />
											<c:forEach var="i" begin="1" end="${delModes}" step="1">
												<c:set var="delMode"
													value="${deliveryModeDataMap.value[delModes-i]}" />

												<c:if test="${count==1}">
													<form:input type="hidden"
														path="deliveryMethodEntry[${entry.entryNumber}].sellerArticleSKU"
														value="${delMode.sellerArticleSKU}" />
													<form:input id="radio_${entry.entryNumber}" type="hidden"
														path="deliveryMethodEntry[${entry.entryNumber}].deliveryCode"
														value="${delMode.code}" />
												</c:if>
												...
												<c:choose>
													<c:when test="${delMode.code eq 'home-delivery'}">
														<li class="${delMode.code }"><input type="radio"
															name="${entry.entryNumber}"
															value="${delMode.deliveryCost.value}"
															id="radio_${entry.entryNumber}_${delMode.code}"
															onclick="return calculateDeliveryCost('radio_${entry.entryNumber}','${delMode.code}');"
															checked="checked" /> <label class="deliveryModeLabel"
															for="radio_${entry.entryNumber}_${delMode.code }">${delMode.name }
																- <format:price priceData="${delMode.deliveryCost}"
																	displayFreeForZero="TRUE" />
														</label> <span> ${delMode.description }.</span></li>
