	...
	<li class="item" id="entry-${entry.entryNumber}">
		<ul class="desktop">
			...
			<li>
												...
												<c:when test="${not empty entry.productPerDiscDisplay}">
													<c:choose>
														<c:when test="${not empty entry.productLevelDisc}">
															<!-- TPR-970 changes--><span class="totalPrice" id="itemCartCentDisplay_${entry.entryNumber}"><span class="off-bag">${entry.productPerDiscDisplay.value}<spring:theme code="off.item.percentage"/><div   class="delSeat"><format:price priceData="${entry.netSellingPrice}"/></div></span></span>
														</c:when>
														<c:otherwise>
															<!-- TPR-970 changes--><span class="totalPrice" id="itemCartCentDisplay_${entry.entryNumber}"><span class="off-bag">${entry.productPerDiscDisplay.value}<spring:theme code="off.item.percentage"/><div   class="delSeat"><format:price priceData="${entry.totalPrice}"/></div></span></span>
														</c:otherwise>
													</c:choose>
												</c:when>
												<c:otherwise>
										...
										<c:otherwise>
											<c:choose>
												<c:when test="${not empty entry.productPerDiscDisplay}">
													<c:choose>
														<c:when test="${not empty entry.productLevelDisc}">
															<!-- TPR-970 changes--><span  class="totalPrice" id="itemCartAmtDisplay_${entry.entryNumber}"><span class="off-bag">${entry.productPerDiscDisplay.value}<spring:theme code="off.item.percentage"/><format:price priceData="${entry.netSellingPrice}"/></span></span>
														</c:when>
														<c:otherwise>
															<!-- TPR-970 changes--><span class="totalPrice" id="itemCartAmtDisplay_${entry.entryNumber}"><span class="off-bag zzz">${entry.productPerDiscDisplay.value}<spring:theme code="off.item.percentage"/><format:price priceData="${entry.totalPrice}"/></span></span>
														</c:otherwise>
													</c:choose>
												</c:when>
												<c:otherwise>
										...
										<c:choose>
											<c:when test="${not empty entry.cartLevelDisc && not empty entry.cartLevelPercentage}">
												<c:if test="${entry.amountAfterAllDisc.value gt 0.1}">
													<!-- TPR-970 changes--><span class="totalPrice" id="cartCentOfferDisplay_${entry.entryNumber}"><span class="off-bag">${entry.cartLevelPercentage}<spring:theme code="off.bag.percentage"/><format:price priceData="${entry.amountAfterAllDisc}"/></span></span>
											...
											<c:otherwise>
												<c:if test="${entry.amountAfterAllDisc.value gt 0.1}">
													<span class="totalPrice" id="cartAmtOfferDisplay_${entry.entryNumber}"><span class="off-bag"><format:price priceData="${entry.cartLevelDisc}"/><spring:theme code="off.bag"/><format:price priceData="${entry.amountAfterAllDisc}"/></span></span>
				...
				<div class="itemsRemove-Move mt-10 mb-20">
					<c:if test="${entry.updateable}">
						...
						<ycommerce:testId code="cart_product_removeProduct">
							...
							<form:form name="addToCartForm" method="post" action="#" class="mybag-undo-form">
								...
								<div class="undo-text-wrapper" style="display : none">
					...
					<c:if test="${!entry.giveAway}">
						...
						<a class="normalSizeMediumTxt move-to-wishlist-button cart_move_wishlist" id="moveEntry_${entry.entryNumber}" onclick="openPopFromCart('${entry.entryNumber}','${entry.product.code}','${entry.selectedSellerInformation.ussid}');" data-toggle="popover" data-placement='bottom'><spring:theme code="basket.move.to.wishlist"/></a>
					</c:if>
				</div>
			</li>
			...
			<li id ="${entry.selectedSellerInformation.ussid}_li_${entry.giveAway}" class="delivery freebie-delivery">
				<ul id="${entry.selectedSellerInformation.ussid}_${entry.giveAway}">
					...
					<c:otherwise>
					<li id ="${entry.selectedSellerInformation.ussid}_li" class="delivery" style="display : none">
