										...
										<c:when test="${(facetData.code eq 'colour' || facetData.code  eq 'dialColourWatches' || facetData.code eq 'dialColour-classification') && not empty facetValue.name }">						
											...
											<c:choose>
												<c:when test="${colorAry[0]=='Multi' || colorAry[0]=='multi'}">
													<input type="checkbox" value="${facetValue.code}" name="" id=""><label for="">${facetValue.name}</label>
											...
											<c:if test="${facetData.multiSelect}">											
												<ycommerce:testId code="facetNav_selectForm">							
													...
													<c:choose>
														...
														<c:otherwise>
															<input type="checkbox" ${facetValue.selected ? 'checked="checked"' : ''} value="${facetValue.code}" name="" id=""><label for="">${facetValue.name}</label>
															<span class="avail-count">${facetValue.count}</span>
															<%-- <input type="hidden" name="facetValue" value="${facetValue.code}"/> 
															<c:if test="${facetData.code ne 'dialColour'}"> <!-- Added for TISPRO-490  -->		
																<label>
																	<input type="checkbox" ${facetValue.selected ? 'checked="checked"' : ''} />
																	<c:if test="${facetData.code == 'inStockFlag' && facetValue.code == 'true' && facetStockSize=='2'}">
																		<span class="facet-label">
																			<span class="facet-mark"></span>
																			<div class="facet-text">
																				<span class="facet-text">
