...
<li class="product-item heroProducts"><ycommerce:testId
...
<c:if
					...
					<c:choose>
						<c:when test="${fn:length(product.swatchColor)>6}">
							<c:forEach items="${product.swatchColor}" var="swatchColor" begin="1" end="6">
							  <li><span
									style="background-color: ${swatchColor};border: 1px solid rgb(204, 211, 217);"
									title="${swatchColor}"></span></li>
							</c:forEach>
							+${fn:length(product.swatchColor)-6}&nbsp;more
						</c:when>
						<c:otherwise>
			...
			<c:if
				...
				<div class="compare">
					<input type="checkbox" id="${product.code}"> <label
						for="${product.code}"></label>
					<div class="compareText" id="compare${product.code}">Add to
						compare</div>
				</div>
			</c:if>
		</div>
