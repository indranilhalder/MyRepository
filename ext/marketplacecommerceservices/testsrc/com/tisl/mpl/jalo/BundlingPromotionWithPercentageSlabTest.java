...
public class BundlingPromotionWithPercentageSlabTest extends MplServicelayerTest
{
	...
	//SONAR FIX
	/*
	 * @Autowired private CommerceCartService commerceCartService;
	 */
	...
	private void promotionSetup(final ProductModel product)
	{
		...
		final BundlingPromotionWithPercentageSlabModel dicountModel = new BundlingPromotionWithPercentageSlabModel();
		...
		countListData.put(Long.valueOf(1), Double.valueOf(2));
		countListData.put(Long.valueOf(2), Double.valueOf(5));
		countListData.put(Long.valueOf(3), Double.valueOf(10));
	...
	private Collection<PromotionQuantityAndPricesRowModel> getPriceRowData(final Map<Long, Double> countListData)
	{
		...
		if (MapUtils.isNotEmpty(countListData))
		{
			for (final Map.Entry<Long, Double> entry : countListData.entrySet())
			{
				...
				oModel.setPercentageDiscount(entry.getValue());
				dataList.add(oModel);
			}
		}
		return dataList;
