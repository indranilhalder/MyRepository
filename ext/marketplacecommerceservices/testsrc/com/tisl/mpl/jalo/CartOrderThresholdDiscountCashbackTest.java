 ...
 *
 */
public class CartOrderThresholdDiscountCashbackTest
{
	@Resource
	...
	private CartOrderThresholdDiscountCashbackModel preparePromotion(final String code, final double thresholdTotals,
			final Double percentageDiscount, PromotionGroupModel promotionGroup, final double discountPrices)
	{
		//Setting up the Promotion with all details
		final CartOrderThresholdDiscountCashbackModel promo = modelService.create(CartOrderThresholdDiscountCashbackModel.class);
		promo.setCode(code);
	...
	public void testHighDiscountPromotion()
	{
		...
		promotions.add(preparePromotion("discount 1k", 100.00, Double.valueOf(10.00), promotionGroup, 30));
	...
	public void testLowDiscountPromotion()
	{
		...
		final CartModel cart = cartService.getSessionCart();
		final List<AbstractPromotionModel> promotions = new ArrayList<AbstractPromotionModel>();
		promotions.add(preparePromotion("discount 10", 10.00, Double.valueOf(10.00), promotionGroup, 10));
		promotionGroup.setPromotions(promotions);
		cartService.addNewEntry(cart, product, 1, product.getUnit());
		modelService.save(cart);
		promotionsService.updatePromotions(Collections.singletonList(promotionGroup), cart, false, AutoApplyMode.APPLY_ALL,
