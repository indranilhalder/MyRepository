...
public class MplDeliveryCostDaoImpl implements MplDeliveryCostDao
{
			...
			final String sellerArticleSku, final String fulfillmentType)
	{
		...
		MplZoneDeliveryModeValueModel mplZoneDeliveryModeValueModel = null;
		if (StringUtils.isNotEmpty(deliveryCode) && StringUtils.isNotEmpty(currencyIsoCode)
				&& StringUtils.isNotEmpty(sellerArticleSku) && StringUtils.isNotEmpty(fulfillmentType))
		{
			PK deliveryPk = null;
			...
			if (deliveryPk != null)
			{
				...
				{
					//mplZoneDeliveryModeValueModel = searchRes1.getResult().get(0);
					for (final MplZoneDeliveryModeValueModel zonevalue : searchRes1.getResult())
					{
						if (zonevalue.getDeliveryFulfillModes().getCode().equalsIgnoreCase(fulfillmentType))
						{
							mplZoneDeliveryModeValueModel = zonevalue;
						}
					}
				}
			}
