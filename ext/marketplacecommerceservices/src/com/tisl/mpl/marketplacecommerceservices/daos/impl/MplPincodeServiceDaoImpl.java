...
public class MplPincodeServiceDaoImpl implements MplPincodeServiceDao
{
	...
	/**
	 * Fetch Order Data to generate Report
	 *
	 * @param sysDate
	 * @param lastRunDate
	 */
	@Override
	public List<OrderModel> fetchOrderList(final Date lastRunDate, final Date sysDate)
	{
		try
		{
			String queryString = MarketplacecommerceservicesConstants.EMPTY;
			...
			if (null != lastRunDate)
			{
				queryString = "SELECT {order.pk} FROM {Order as order} WHERE ( {order.modifiedtime} between ?jobTime and ?currenttime ) and {isFallBack} = ?isFallback";
				flag = true;
			}
			else
			{
				queryString = "SELECT {order.pk} FROM {Order as order} WHERE {isFallBack} = ?isFallback ";
			}
			...
			if (flag)
			{
				query.addQueryParameter("jobTime", lastRunDate);
				query.addQueryParameter("currenttime", sysDate);
				query.addQueryParameter("isFallback", "1");
			}
			else
			{
				query.addQueryParameter("isFallback", "1");
			...
			final List<OrderModel> dataList = flexibleSearchService.<OrderModel> search(query).getResult();
			if (CollectionUtils.isNotEmpty(dataList))
			{
