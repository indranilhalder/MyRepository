...
public class DefaultJuspayWebHookServiceImpl implements JuspayWebHookService
{
	...
	private void processWebhook(final JuspayWebhookModel hook, final List<JuspayWebhookModel> webHookDetailList)
	{
		...
		//TISPRO-607
		/*
		 * if (null != hook.getOrderStatus()) { // final OrderModel order =
		 * getMplPaymentService().fetchOrderOnGUID(hook.getOrderStatus().getOrderId()); // if ((null !=
		 * order.getIsWallet() && WalletEnum.NONWALLET.toString().equals(order.getIsWallet().getCode())) // ||
		 * order.getIsWallet() == null) // {**********Commented for mRupee if
		 * (CollectionUtils.isEmpty(hook.getOrderStatus().getRefunds())) { //For Positive Flow // final OrderModel order =
		 * getMplPaymentService().fetchOrderOnGUID(hook.getOrderStatus().getOrderId()); // if
		 * (order.getIsWallet().getCode().equals(WalletEnum.NONWALLET.toString())) // { getResponseBasedOnStatus(hook,
		 * hook.getOrderStatus().getOrderId(), hook.getOrderStatus().getStatus()); //} } //For Refund Flow else { //Action
		 * for refund scenarios performActionForRefund(webHookDetailList); //Processed in Webhook
		 * updateWebHookExpired(hook); } // } }
		 */
		if (null != juspayOrderStatus)
		{
			if (CollectionUtils.isEmpty(juspayOrderStatus.getRefunds()))
			{
				//For Positive Flow
				getResponseBasedOnStatus(hook, juspayOrderStatus.getOrderId(), juspayOrderStatus.getStatus());
			}
			//For Refund Flow
			else
			{
		...
		for (final JuspayWebhookModel hook : webHookList)
		{
			if (null != hook.getOrderStatus() && CollectionUtils.isNotEmpty(hook.getOrderStatus().getRefunds()))
			{
				for (final JuspayRefundResponseModel refund : hook.getOrderStatus().getRefunds())
				{
					...
					{
						//TISPRO-607
						if (StringUtils.isEmpty(refund.getUniqueRequestId()))
						{
							isError = true;
							errorList.add(refund.getId());

							throw new Exception(MarketplacecommerceservicesConstants.E0021);
						}
						else
						{
							...
							if (CollectionUtils.isNotEmpty(results))
							{
								...
								{
									if (!rtmModel.getIsProcessed().booleanValue() && null != hook.getOrderStatus().getOrderId())
									{
										...
										if (null != subOrder
												&& null != rtmModel.getRefundType()
												&& rtmModel.getRefundType().toString()
														.equalsIgnoreCase(JuspayRefundType.CANCELLED.toString()))
										{
											//Change status of Consignment against order line when it is SUCCESS at Juspay & CANCELLED in RTM
											changeConsignmentStatusForCancelled(rtmModel, refund, subOrder, hook.getOrderStatus()
													.getOrderId());
										}
										...
										// added in R2.3 for refund Delivery / schedule Delivery Charges Start
										else if (null != subOrder && null != rtmModel.getRefundType()
												&& rtmModel.getRefundType().equals(JuspayRefundType.REFUND_DELIVERY_CHARGE))
										{
											try
											{
												createPaymentTransactionModelForDeliveryCharges(rtmModel, refund, subOrder, hook
														.getOrderStatus().getOrderId());
											}
											catch (final Exception e)
											{
												LOG.error("Exception occurred while creating paymentTransactionModel for DeliveryCharges for order id "
														+ subOrder.getCode());
											}
										}
										else if (null != subOrder && null != rtmModel.getRefundType()
												&& rtmModel.getRefundType().equals(JuspayRefundType.REFUND_SCHEDULE_DELIVERY_CHARGE))
										{
											try
											{
												createPaymentTransactionModelForScheduleDelCharges(rtmModel, refund, subOrder, hook
														.getOrderStatus().getOrderId());
											}
											catch (final Exception e)
											{
										...
										// added in R2.3 for refund Delivery / schedule Delivery Charges End
									}
								}
							}
							else
							{
								if (null != hook.getOrderStatus().getOrderId())
								{
									...
									if (null != refund.getUniqueRequestId()
											&& refund.getUniqueRequestId().equalsIgnoreCase(entry.getRefundReqId()))
									{
												...
												&& refund.getStatus().equalsIgnoreCase(MarketplacecommerceservicesConstants.SUCCESS))
										{
											...
											paymentTransactionEntryModel
													.setTransactionStatusDetails(MarketplacecommerceservicesConstants.SUCCESS);
											...
											&& refund.getStatus().equalsIgnoreCase(MarketplacecommerceservicesConstants.FAILURE))
									{
										...
										paymentTransactionEntryModel
												.setTransactionStatusDetails(MarketplacecommerceservicesConstants.FAILURE);
										paymentTransactionEntryModel.setType(PaymentTransactionType.REFUND_STANDALONE);
	 ...
	 * @param juspayOrderId
	 */
	private void createPaymentTransactionModelForDeliveryCharges(final RefundTransactionMappingModel rtmModel,
			final JuspayRefundResponseModel refund, final OrderModel order, final String juspayOrderId)
	{

		LOG.info("Creating  PaymentTransactionModel for DeliveryCharges refund");
		try
		{
			if (null != rtmModel.getJuspayRefundId() && null != rtmModel.getRefundedOrderEntry()
					&& null != rtmModel.getRefundedOrderEntry().getTransactionID())
			{
				...
				{
					paymentTransactionModel = getMplJusPayRefundService().createPaymentTransactionModel(order,
							MarketplacecommerceservicesConstants.SUCCESS, refundAmount, PaymentTransactionType.REFUND_DELIVERY_CHARGES,
				...
				{
					paymentTransactionModel = getMplJusPayRefundService().createPaymentTransactionModel(order,
							MarketplacecommerceservicesConstants.FAILURE, refundAmount, PaymentTransactionType.REFUND_DELIVERY_CHARGES,
	 ...
	 * @param juspayOrderId
	 */
	private void createPaymentTransactionModelForEdCharges(final RefundTransactionMappingModel rtmModel,
			final JuspayRefundResponseModel refund, final OrderModel order, final String juspayOrderId)
	{
		...
		try
		{
					...
					&& null != rtmModel.getRefundedOrderEntry().getTransactionID())
			{
				...
				if (newStatus != null)
				{
					...
					orderEntryModel.setEdChargesJuspayRequestId(uniqueRequestId);
					getModelService().save(orderEntryModel);
	 ...
	 * @param juspayOrderId
	 */
	private void createPaymentTransactionModelForScheduleDelCharges(final RefundTransactionMappingModel rtmModel,
			final JuspayRefundResponseModel refund, final OrderModel order, final String juspayOrderId)
	{
		try
		{
					...
					&& null != rtmModel.getRefundedOrderEntry().getTransactionID())
			{
				...
				if (refund.getStatus().equalsIgnoreCase(MarketplacecommerceservicesConstants.SUCCESS))
				{
					paymentTransactionModel = getMplJusPayRefundService().createPaymentTransactionModel(order,
							MarketplacecommerceservicesConstants.SUCCESS, refundAmount,
							PaymentTransactionType.REFUND_SCHEDULE_DELIVERY_CHARGES, REFUND, uniqueRequestId);
					if (null != rtmModel.getRefundedOrderEntry() && null != rtmModel.getRefundedOrderEntry().getConsignmentEntries())
					{
						newStatus = rtmModel.getRefundedOrderEntry().getConsignmentEntries().iterator().next().getConsignment()
								.getStatus();
				...
				else if (refund.getStatus().equalsIgnoreCase(MarketplacecommerceservicesConstants.FAILURE))
				{
					paymentTransactionModel = getMplJusPayRefundService().createPaymentTransactionModel(order,
							MarketplacecommerceservicesConstants.FAILURE, refundAmount,
							PaymentTransactionType.REFUND_SCHEDULE_DELIVERY_CHARGES, REFUND_FAIL, uniqueRequestId);
	...
	private String getOdrStatus(final String orderGuid) throws EtailNonBusinessExceptions
	{
		return getJuspayWebHookDao().fetchStatusOnGUID(orderGuid);
