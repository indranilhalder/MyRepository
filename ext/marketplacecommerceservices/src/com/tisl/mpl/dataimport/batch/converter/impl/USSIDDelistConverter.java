...
import com.tisl.mpl.core.cronjobs.delisting.DelistingProcessor;


/**
 * Default implementation of {@link ImpexConverter}.
 */
public class USSIDDelistConverter extends DefaultImpexConverter
{
	@Autowired
	...
	private static Integer ISPROCESSEDCOLOUMN = Integer.valueOf("3");
	...
	{
		//Calling Delisting Processor to delist the USSID
		final boolean processed = mplDelistingProcessor.processDatatoModelUssid(row);

		//Inserting boolean Value using hot Folder
		if (processed)
		{
			//true->1
			row.put(ISPROCESSEDCOLOUMN, "1");
		}
		else
		{ //false->0
			row.put(ISPROCESSEDCOLOUMN, "0");
		}
