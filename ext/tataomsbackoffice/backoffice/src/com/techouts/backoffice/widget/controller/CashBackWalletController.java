...
public class CashBackWalletController extends DefaultWidgetController
{
			...
			for (final CilqCashWalletPojo walletObj : cilqCashWalletPojoList)
			{
				...
				if (null != currentCustomer)
				{
					...
					if (null != currentCustomer.getIsWalletActivated() && currentCustomer.getIsWalletActivated().booleanValue())
					{
						...
						{
							request.setNotes("Sample load for " + walletObj.getAmount() + " for PROMOTION");
						...
						{
							request.setNotes("Sample load for " + walletObj.getAmount() + " for CASHBACK");
							response = mplWalletFacade.addTULWalletCashBack(currentCustomer.getCustomerWalletDetail().getWalletId(),
									request);
						}
					...
					else
					{
						try
						{
							...
							if (customerRegisterResponse.getResponseCode() == 0)
							{
								...
								{
									request.setNotes("Sample load for " + walletObj.getAmount() + " for PROMOTION");
									response = mplWalletFacade.createPromotion(currentCustomer.getCustomerWalletDetail().getWalletId(),
											request);
								}
								else if (walletObj.getBucketName().equalsIgnoreCase("CASHBACK"))
								{
									request.setNotes("Sample load for " + walletObj.getAmount() + " for CASHBACK");
									response = mplWalletFacade
											.addTULWalletCashBack(currentCustomer.getCustomerWalletDetail().getWalletId(), request);
								}
								commentMsg = "Success";
							}
							else
				...
				if (!responseCheck)
				{
					...
					if (null != response.getCards() && response.getCards().size() > 0)
					{
						...
						pojo.setCardNumber(response.getCards().get(0).getExpiry());
					}
				}
				else
				{
	...
	public List<String> parseLine(final String cvsLine)
	{
		return parseLine(cvsLine, DEFAULT_SEPARATOR, DEFAULT_QUOTE);
	}

	public List<String> parseLine(final String cvsLine, char separators, char customQuote)
	{
