...
public class CashBackWalletController extends DefaultWidgetController
{
	...
	public void selectUploadWallet(final UploadEvent uploadEvent) throws FileNotFoundException
	{
		...
		if (null != configurationService)
		{
			...
			if (fileName.endsWith(CSV) || fileName.endsWith(XLSX))
			{
				...
				final List<CilqCashWalletPojo> cilqCashWalletPojoList = new ArrayList<CilqCashWalletPojo>();
				final Scanner scanner = new Scanner(media.getStreamData());
				try
				{
					while (scanner.hasNext())
					{
						final CilqCashWalletPojo pojo = new CilqCashWalletPojo();
						final List<String> line = CSVFileReaderUtils.parseLine(scanner.nextLine());
						try
						{
							...
							if (null != line.get(0) && !line.get(0).isEmpty() && null != line.get(1) && !line.get(1).isEmpty()
									&& null != line.get(2) && !line.get(2).isEmpty() && line.get(2).contains("@") && null != line.get(3)
									&& !line.get(3).isEmpty() && null != line.get(4) && !line.get(4).isEmpty())
							{
						...
						{
							Messagebox.show(
									"Please upload file valid format like  First Name,Last Name ,Customer Email ,Amount , Bucket Type ",
									"Error Message", Messagebox.OK, Messagebox.ERROR);
							messageFlag = true;
						}
					}
				}
				catch (final Exception eObj)
				{
				...
				if (null != fileUploadLocation && !fileUploadLocation.isEmpty() && null != fileUploadName)
				{
					try
					{
						...
						final StringBuilder builder = uploadWallettCashFile(cilqCashWalletPojoList, pw);
	...
	private StringBuilder uploadWallettCashFile(final List<CilqCashWalletPojo> cilqCashWalletPojoList, final PrintWriter pw)
	{
		...
		for (final CilqCashWalletPojo walletObj : cilqCashWalletPojoList)
		{
			final CustomerModel currentCustomer = extendedUserService.getUserForOriginalUid(walletObj.getCustomerEmailId());
			if (null != currentCustomer)
			{
				...
				else if (null != currentCustomer.getCustomerWalletDetail() && null != currentCustomer.getIsWalletActivated()
						&& currentCustomer.getIsWalletActivated().booleanValue() == false)
				{
					/*
					 * try{ response = addAmountToQCWallet(currentCustomer,walletObj);
					 * currentCustomer.setFirstName(walletObj.getCustomerFirstName());
					 * currentCustomer.setLastName(walletObj.getCustomerLastName());
					 * currentCustomer.setIsWalletActivated(Boolean.TRUE);
					 * currentCustomer.setQcVerifyFirstName(walletObj.getCustomerFirstName());
					 * currentCustomer.setQcVerifyLastName(walletObj.getCustomerLastName());
					 * modelService.save(currentCustomer); if(null != response && response.getResponseCode()== 0){ commentMsg
					 * =SUCCESS_MSG; }else{ commentMsg = qcErrorMasseges(response.getResponseCode()); } }catch (final
					 * Exception ex){ ex.printStackTrace(); }
					 */
					commentMsg = "Currently User is DeActivated Please activate ";
				}
				else if (currentCustomer.getCustomerWalletDetail() == null && null != currentCustomer.getIsWalletActivated()
	...
	private QCRedeeptionResponse addAmountToQCWallet(final CustomerModel currentCustomer, final CilqCashWalletPojo walletObj)
	{
		...
		try
		{
			final QCCustomerPromotionRequest request = new QCCustomerPromotionRequest();
	...
	private String createQCWalletForCustomer(final CustomerModel currentCustomer, final CilqCashWalletPojo walletObj)
	{
		...
		try
		{
			...
			if (null != customerRegisterResponse && customerRegisterResponse.getResponseCode() == 0)
			{
	...
	private String qcErrorMasseges(final Integer qcErrorCode)
	{
		String commentMsg = null;
	...
	public void cancelUploadWallet(final UploadEvent uploadEvent) throws FileNotFoundException
	{
		...
		if (fileName.endsWith(CSV) || fileName.endsWith(XLSX))
		{
			...
			while (scanner.hasNext())
			{
				...
				{
					Messagebox.show("Please upload file valid format like  Customer Email ,Transaction Id ", "Error Message",
			...
			for (final CilqCashWalletPojo walletObj : cilqCashWalletPojoList)
			{
				...
				if (null != currentCustomer)
				{
					...
					{
						responseObj = mplWalletFacade.refundTULPromotionalCash(currentCustomer.getCustomerWalletDetail().getWalletId(),
								walletObj.getTransactionId());
		...
		if (fileName.endsWith(CSV) || fileName.endsWith(XLSX))
		{
			...
			try
			{
				while (scanner.hasNext())
				{
					...
					{
						Messagebox.show("Please upload file valid format like  Customer Wallet Id ,Customer Email ,Remarks ",
								"Error Message", Messagebox.OK, Messagebox.ERROR);
						messageFlag = true;
					}
				}
			...
			{
				fileUploadLocation = configurationService.getConfiguration().getString("mpl.wallet.bulkuploadfileLocation");
				fileUploadName = configurationService.getConfiguration()
						.getString("mpl.wallet.bulkupload.activateanddeactivate.filename");
				if (null != fileUploadLocation && !fileUploadLocation.isEmpty() && null != fileUploadName)
				{
					try
					{
						...
						pw = new PrintWriter(new File(fileUploadLocation + fileUploadName));
						final StringBuilder builder = activateAndDeactivateQCAccounts(cilqCashWalletPojoList, pw);
						pw.write(builder.toString());
						pw.close();
					}
					catch (final FileNotFoundException e)
	...
	private StringBuilder activateAndDeactivateQCAccounts(final List<CilqCashWalletPojo> cilqCashWalletPojoList,
			final PrintWriter pw)
	{
		...
		for (final CilqCashWalletPojo walletObj : cilqCashWalletPojoList)
		{
			...
			if (null != currentCustomer)
			{
				...
				else if (null != currentCustomer.getIsWalletActivated()
						&& currentCustomer.getIsWalletActivated().equals(Boolean.FALSE))
				{
	...
	public void qcAccountActiveAndDeactiveUploadedCsv() throws InterruptedException, FileNotFoundException
	{
		...
		{
			fileUploadLocation = configurationService.getConfiguration().getString("mpl.wallet.bulkuploadfileLocation");
			fileDownloadName = configurationService.getConfiguration()
					.getString("mpl.wallet.bulkupload.activateanddeactivate.filename");
			if (null != fileUploadLocation && !fileUploadLocation.isEmpty() && null != fileDownloadName)
			{
				final File file = new File(fileUploadLocation.trim() + fileDownloadName);
