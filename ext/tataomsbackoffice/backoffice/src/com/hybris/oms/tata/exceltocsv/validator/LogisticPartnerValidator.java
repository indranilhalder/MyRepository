...
{
	private static final Logger LOG = LoggerFactory.getLogger(LogisticPartnerValidator.class);
			...
			final ArrayList<LogisticPartner> logisticPartners, final FileWriter writer, final int rowNumber) throws IOException
	{
		...
		for (final LogisticPartner logisticPartner : logisticPartners)
		{
			...
			csvService.addLogisticPartnerData(deliveryMode, logisticPartner, priorityMaster);
		}
	}
			...
			final FileWriter writer, final int rowNumber) throws IOException
	{
		...
		else if (!formReuired.equalsIgnoreCase("YES,") && !formReuired.equalsIgnoreCase("NO,") && formReuired.length() > 1)
		{
			writer.write(ExcelConstants.ROW_VALUE + ExcelConstants.EMPTY_SPACE + rowNumber + ExcelConstants.EMPTY_SPACE
					+ logisticPartner.getName() + ExcelConstants.FORM_REQUIRED_VALIDATION + ExcelConstants.NEW_LINE);
		}
	}
			...
			final FileWriter writer, final int rowNumber) throws IOException
	{
		...
		{
			writer.write(ExcelConstants.ROW_VALUE + ExcelConstants.EMPTY_SPACE + rowNumber + ExcelConstants.EMPTY_SPACE
					+ logisticPartner.getName() + ExcelConstants.PICKUP_VALIDATION + ExcelConstants.NEW_LINE);
		}
		...
		if (logisticPartner.getName().equalsIgnoreCase(priorityMaster))
		{
			...
			if (!prepaidLimit.equals(","))
			{
				...
				if (result)
				{
					...
					{
						writer.write(ExcelConstants.ROW_VALUE + ExcelConstants.EMPTY_SPACE + rowNumber + ExcelConstants.EMPTY_SPACE
								+ logisticPartner.getName() + ExcelConstants.PREPAID_LIMIT_MANDITORY + ExcelConstants.NEW_LINE);
					}
				} // if condition
				else
				{
			...
			final FileWriter writer, final int rowNumber) throws IOException
	{
		if (logisticPartner.getName().equalsIgnoreCase(priorityMaster))
		{
			...
			{
				writer.write(ExcelConstants.ROW_VALUE + ExcelConstants.EMPTY_SPACE + rowNumber + ExcelConstants.EMPTY_SPACE
						+ logisticPartner.getName() + ExcelConstants.COD_COLUMN_VALIDATION + ExcelConstants.NEW_LINE);
			}
		} //if condition for checking priority equal or not with logisticpartner name
		...
		if (logisticPartner.getName().equalsIgnoreCase(priorityMaster))
		{
			...
			if (!codLimit.equals(","))
			{
				...
				if (result)
				{
					...
					{
						writer.write(ExcelConstants.ROW_VALUE + ExcelConstants.EMPTY_SPACE + rowNumber + ExcelConstants.EMPTY_SPACE
								+ logisticPartner.getName() + ExcelConstants.COD_LIMIT_MANDITORY + ExcelConstants.NEW_LINE);
					}
				} // if condition
				else
				{
