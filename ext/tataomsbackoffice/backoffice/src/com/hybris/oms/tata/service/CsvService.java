...
public class CsvService
{
	...
	{
		final int codcolumn = Integer.parseInt(ExcelConstants.COD_COLUMN_INDEX);
		final int surfaceModeStartingIndex = Integer.parseInt(ExcelConstants.SURFACE_MODE_START_INDEX);
		...
		{
			if (logisticPartner.getName().equalsIgnoreCase(ExcelConstants.BLUDART_OUTPUT_NAME)
			...
			else if (logisticPartner.getName().equalsIgnoreCase(ExcelConstants.GOJAVAS_OUTPUT_NAME)
					&& !logisticPartner.getPrepaidPriority().equals(",") || !logisticPartner.getCodPriority().equals(","))
			{
				appendData(deliveryMode, logisticPartner, data1, priorityMaster);

			}
			else if (logisticPartner.getName().equalsIgnoreCase(ExcelConstants.FEDEX_OUTPUT_NAME)
					&& !logisticPartner.getPrepaidPriority().equals(",") || !logisticPartner.getCodPriority().equals(","))
			{
				appendData(deliveryMode, logisticPartner, data2, priorityMaster);

			}
			else if (logisticPartner.getName().equalsIgnoreCase(ExcelConstants.GATI_OUTPUT_NAME)
					&& !logisticPartner.getPrepaidPriority().equals(",") || !logisticPartner.getCodPriority().equals(","))
			{
				appendData(deliveryMode, logisticPartner, data3, priorityMaster);
			}
			else if (logisticPartner.getName().equalsIgnoreCase(ExcelConstants.SHADOW_OUTPUT_NAME)
					&& !logisticPartner.getPrepaidPriority().equals(",") || !logisticPartner.getCodPriority().equals(","))
			{
				appendData(deliveryMode, logisticPartner, data4, priorityMaster);

			}
			else if (logisticPartner.getName().equalsIgnoreCase(ExcelConstants.NUVOEX_OUTPUT_NAME)
					&& !logisticPartner.getPrepaidPriority().equals(",") || !logisticPartner.getCodPriority().equals(","))
			{
				appendData(deliveryMode, logisticPartner, data5, priorityMaster);

			}
			else if (logisticPartner.getName().equalsIgnoreCase(ExcelConstants.ECOMEXPRESS_OUTPUT_NAME)
					&& !logisticPartner.getPrepaidPriority().equals(",") || !logisticPartner.getCodPriority().equals(","))
			{
				appendData(deliveryMode, logisticPartner, data6, priorityMaster);

			}
			else if (logisticPartner.getName().equalsIgnoreCase(ExcelConstants.DELHIVERY_OUTPUT_NAME)
					&& !logisticPartner.getPrepaidPriority().equals(",") || !logisticPartner.getCodPriority().equals(","))
			{
				appendData(deliveryMode, logisticPartner, data7, priorityMaster);
			}
			else if (logisticPartner.getName().equalsIgnoreCase(ExcelConstants.EKART_OUTPUT_NAME)
					&& !logisticPartner.getPrepaidPriority().equals(",") || !logisticPartner.getCodPriority().equals(","))
			{
	...
	{
		final int surfaceModeStartingIndex = Integer.parseInt(ExcelConstants.SURFACE_MODE_START_INDEX);
		logisticdata.append(deliveryMode + ExcelConstants.ADD_COMMA);
		logisticdata.append(priorityMaster.getPincode());
		if (logisticPartner.getIndex() < surfaceModeStartingIndex)
		{
			logisticdata.append(ExcelConstants.AIR_MODE + ",");
		}
		else
		{
			logisticdata.append(ExcelConstants.SURFACE_MODE + ",");
		}
	...
	public void appendHeaderData(final String deliveryMode)
	{
		...
		final String headerColumns = ExcelConstants.HEADER_COLUMNS + ExcelConstants.NEW_LINE;
		headerData.append(headerColumns);
		String result = null;
		if (deliveryMode.equalsIgnoreCase(ExcelConstants.DATA_MODE_TYPE1))
		{
			final String STATICDATAHD = ExcelConstants.STATICDATA_HD;
			headerData.append(STATICDATAHD);
			result = addCommasToBOF(ExcelConstants.ADD_COMMA);
			headerData.append(result);
			headerData.append(ExcelConstants.COUNTRY);
			headerData.append(ExcelConstants.NEW_LINE);
		}
		else if (deliveryMode.equalsIgnoreCase(ExcelConstants.DATA_MODE_TYPE2))
		{
			final String STATICDATAED = ExcelConstants.STATICDATA_ED;
	...
	public void appendFooterData(final String deliveryMode, final String fileNameTimeStamp)
	{
		...
		String result = null;
		footerData.append(deliveryMode + ExcelConstants.ADD_COMMA + "EOF," + fileNameTimeStamp + ExcelConstants.ADD_COMMA);
		result = addCommasToEOF(ExcelConstants.ADD_COMMA);
	...
	{
		final String commasCountOfBOF = ExcelConstants.COMMAS_COUNTOF_BOF;
		...
		{
			staticDataHd.append(ExcelConstants.ADD_COMMA);
		}
	...
	{
		final String commasCountOfEOF = ExcelConstants.COMMAS_COUNTOF_EOF;
		final int commasCount = Integer.parseInt(commasCountOfEOF);
		final StringBuffer staticDataEd = new StringBuffer();
		for (int i = 0; i < commasCount; i++)
		{
			staticDataEd.append(ExcelConstants.ADD_COMMA);
		}
