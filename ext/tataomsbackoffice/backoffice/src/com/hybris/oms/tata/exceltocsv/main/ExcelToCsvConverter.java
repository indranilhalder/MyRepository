...
{
	private static final Logger LOG = LoggerFactory.getLogger(ExcelToCsvConverter.class);

	@Resource(name = "csvService")
	private CsvService csvService;
	@Resource(name = "logisticPartnerValidator")
	private LogisticPartnerValidator logisticPartnerValidator;
	@Resource(name = "priorityMasterValidator")
	private PriorityMasterValidator priorityMasterValidator;
	...
	/**
	 * @param csvService
	 *           the csvService to set
	 */
	public void setCsvService(final CsvService csvService)
	{
	 ...
	 */
	public void setPriorityMasterValidator(final PriorityMasterValidator priorityMasterValidator)
	{
	 ...
	 */
	public void setLogisticPartnerValidator(final LogisticPartnerValidator logisticPartnerValidator)
	{
			...
			final File validationFile, final String fileNameTimeStamp)
	{
		...
		try
		{
			...
			final RowCount rowCount = new RowCount();
			...
			csvService.appendHeaderData(deliveryMode);
			for (final Row row : sheet)
			{
				...
				if (row.getRowNum() == 1)
				{
					final int lastColumnIndex = Integer.parseInt(ExcelConstants.LAST_COLUMN_INDEX);
					final int columnIndex = Integer.parseInt(ExcelConstants.COD_COLUMN_INDEX);
						...
						switch (i)
						{
							case 0:
								...
								if (result)
								{
									...
									{
										writer.write(ExcelConstants.ROW_VALUE + ExcelConstants.EMPTY_SPACE + rowNumber
												+ ExcelConstants.EMPTY_SPACE + ExcelConstants.PINCODE + ExcelConstants.NEW_LINE);
									}
								}
								else
								{
							...
							case 19:
								...
								{
									writer.write(
											ExcelConstants.ROW_VALUE + ExcelConstants.EMPTY_SPACE + rowNumber + ExcelConstants.EMPTY_SPACE
													+ ExcelConstants.EMPTY_SPACE + ExcelConstants.CITY + ExcelConstants.NEW_LINE);
								}
								else
								{
							...
							case 20:
								...
								{
									writer.write(ExcelConstants.ROW_VALUE + ExcelConstants.EMPTY_SPACE + rowNumber
											+ ExcelConstants.EMPTY_SPACE + state + ExcelConstants.EMPTY_SPACE
											+ ExcelConstants.STATE_VALIDATION + ExcelConstants.NEW_LINE);
								}
								else
								{
								...
								switch (propertiesPopulated)
								{
									...
									case 6:
										logisticPartner.setNewzone(
												getCellData(row.getCell(j)) + ExcelConstants.ADD_COMMA + ExcelConstants.ADD_COMMA);
										propertiesPopulated++;
										break;
									case 7:
										logisticPartner.setFormRequired(getCellData(row.getCell(j)) + ExcelConstants.ADD_COMMA);
										propertiesPopulated++;
										break;
									case 8:
			...
			writer.flush();
			writer.close();
			csvService.appendFooterData(deliveryMode, fileNameTimeStamp);
			csvService.writeData(csvService.getData(), outputFile, validationFile);
