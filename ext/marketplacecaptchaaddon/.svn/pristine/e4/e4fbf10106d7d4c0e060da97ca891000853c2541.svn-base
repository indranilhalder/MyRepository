...
/**
 * This is the extension manager of the Marketplacecaptchaaddon extension.
 */
public class MarketplacecaptchaaddonManager extends GeneratedMarketplacecaptchaaddonManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(MarketplacecaptchaaddonManager.class.getName());
	 ...
	 */
	public static MarketplacecaptchaaddonManager getInstance()
	{
		return (MarketplacecaptchaaddonManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
				MarketplacecaptchaaddonConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public MarketplacecaptchaaddonManager() // NOPMD 
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of MarketplacecaptchaaddonManager called.");
		}
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of MarketplacecaptchaaddonManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of MarketplacecaptchaaddonManager called, current tenant: " + getTenant().getTenantID());
		}
	}
