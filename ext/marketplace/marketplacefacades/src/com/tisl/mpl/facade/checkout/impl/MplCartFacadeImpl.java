...
public class MplCartFacadeImpl extends DefaultCartFacade implements MplCartFacade
{
	...
	public void setCartSubTotal(final CartModel cartModel) throws EtailNonBusinessExceptions
	{
		...
		if (cartModel != null)
		{
			...
			for (final AbstractOrderEntryModel entry : entries)
			{
				...
				{
					entryTotal = entry.getQuantity().doubleValue() * entry.getMrp().doubleValue();
					totalPrice += entry.getQuantity().doubleValue() * entry.getBasePrice().doubleValue();
				}
				else
				{
					entryTotal = entry.getQuantity().doubleValue() * entry.getBasePrice().doubleValue();
				}
			...
			{
				totalPrice = totalPrice - discountValue.doubleValue();
			}
					...
					if (null != cartEntryModel && !cartEntryModel.getGiveAway().booleanValue())
					{
										...
										* cartEntryModel.getQuantity().doubleValue()))
						{
							...
							if (cartEntryModel.getCartLevelDisc() != null)
							{
								...
								cartLevelPercentageDiscount = Double.parseDouble((df.format((cartEntryModel.getCartLevelDisc()
										.doubleValue() / cartEntryModel.getNetSellingPrice().doubleValue()) * 100)));
						...
						else if (null != cartEntryModel.getMrp())
						{
							...
							{
								cartLevelPercentageDiscount = Double.parseDouble((df.format((cartEntryModel.getCartLevelDisc()
										.doubleValue() / cartEntryModel.getBasePrice().doubleValue()) * 100)));
								cartEntryModel.setCartAdditionalDiscPerc(Double.valueOf(cartLevelPercentageDiscount));
								LOG.debug("The cart level percentage is" + cartLevelPercentageDiscount);
							}
							//UF-260 ends here
	...
	private CartEntryModel getEGVCartEntry(final EgvDetailsData egvDetailForm, final CartModel cardModel)
	{
		...
		try
		{
			...
			final DeliveryModeModel deliaveryMode = mplDeliveryCostDao.getDelieveryMode(HOME_DELIVERY);
	...
	@Override
	public CartData getGiftCartData(final String guid)
	{

		final CartModel cartModel = mplEGVCartService.getEGVCartModel(guid);
		CartData cartData = null;
		try
		{
			cartData = getCartConverter().convert(cartModel);
