...
public class CancelReturnFacadeImpl implements CancelReturnFacade
{
		...
		try
		{
			...
			for (final AbstractOrderEntryModel abstractOrderEntryModel : orderEntries)
			{
				...
				if (ticketTypeCode.equalsIgnoreCase("R"))
				{
					sendTicketLineItemData.setReturnReasonCode(reasonCode);
					//TPR-5954
					if (null != returnInfoData.getSubReasonCode() && !returnInfoData.getSubReasonCode().isEmpty())
					{
						sendTicketLineItemData.setSubReasonCode(returnInfoData.getSubReasonCode());
						//sendTicketLineItemData.setImageUrl(returnInfoData.getImageUrl());//To-do post image upload
					}
			...
			//TPR-5954
			if (null != returnInfoData.getComments() && !returnInfoData.getComments().isEmpty())
			{
			...
			else if (ticketTypeCode.equalsIgnoreCase("R"))
			{
				...
				if (null != reasonCode)
				{
					...
					else
					{
						...
						orderLineData.setReturnCancelRemarks(reasonDescNew);
					}

				}
				//orderLineData.setReturnCancelRemarks(getReasonDesc(reasonCode));//Removed old code
			...
			final String transactionId) throws OrderCancelException, Exception
	{
	   ...
	   orderCancelRequest.setAmountTORefundForQc(new Double(refundAmountForQc));
		return orderCancelRequest;
	}
	      	 ...
	      	 if (CollectionUtils.isNotEmpty(orderRequestRecord.getOrderEntriesModificationEntries()))
	    			{
	    						...
	    						.getOrderEntriesModificationEntries())
	    				{
	    					...
	    					if (orderEntry != null)
	    					{
	    						...
	    						else if (StringUtils.equalsIgnoreCase(abstractOrderEntryModel.getWalletApportionReturnInfo().getStatus(), "PENDING"))
	    						{
	    						...
	    						LOG.debug("****** initiateRefund : Step 3  >>Payment transaction mode is not null >> Calling OMS with status as received from JUSPAY "
	    								+ newStatus.getCode());
	    						mplJusPayRefundService.makeRefundOMSCall(orderEntry, null, Double.valueOf(decimalFormat.format(orderCancelRequest.getAmountTORefundForQc())), newStatus, null);
			...
			if(null !=abstractOrderEntryModel.getWalletApportionPaymentInfo() && null!= abstractOrderEntryModel.getWalletApportionPaymentInfo().getQcShippingPartValue()){
			 ...
			 if(null != walletCardApportionDetailModelList &&  walletCardApportionDetailModelList.size()>0){
				 ...
				 for(WalletCardApportionDetailModel walletCardApportionDetailModelObj :walletCardApportionDetailModelList){
					...
					//	final WalletCardReturnEntryModel walletCardReturnEntryModel = getModelService().create(WalletCardReturnEntryModel.class);
						...
						//WalletCardApportionDetailModel qcApportionInfo=calculateRefundAmountForQC(abstractOrderEntryModel, Double.valueOf(returnEntryModel.getCardAmount()).doubleValue());
	 ...
	 */
	private void initiateRefund(final OrderModel subOrderModel, final OrderCancelRecordEntryModel orderRequestRecord, final List<WalletCardApportionDetailModel> walletCardApportionDetailModelList,final String transactionId)
	{
					...
					if (CollectionUtils.isNotEmpty(orderRequestRecord.getOrderEntriesModificationEntries()))
					{
								...
								.getOrderEntriesModificationEntries())
						{
							...
							if (orderEntry != null)
							{
								...
								if(null != subOrderModel.getSplitModeInfo() && subOrderModel.getSplitModeInfo().equalsIgnoreCase("Split")){
									returnModel =constructQuickCilverOrderEntryForSplit(walletCardApportionDetailModelList,transactionId,subOrderModel);
			...
			final OrderLineData orderLineData, final SalesApplication salesApplication)
	{
		...
		try
		{
			...
			if (null != abstractOrderEntryModel)
			{
				...
				refundEntryModel.setOrderEntry(abstractOrderEntryModel);
				refundEntryModel.setReturnRequest(returnRequestModel);
				//TPR-5954 || Differentiate between default and category specific codes || Start
				if (null != orderLineData.getReasonCode() && !orderLineData.getReasonCode().isEmpty())
				{

					orderLineData.getReasonCode().trim();
					if (!Character.isLetter(orderLineData.getReasonCode().charAt(0)))
					{//old
					...
					{//new
						final String reasonDescNew = fetchSubReasonDesc(orderLineData.getReasonCode());
						refundEntryModel.setNotes(reasonDescNew);
						refundEntryModel.setReason(RefundReason.valueOf("NA"));
					}
				...
				if (null != orderLineData.getSubReasonCode() && !orderLineData.getSubReasonCode().isEmpty())
				{
					refundEntryModel.setSubReason(fetchSubReasonDesc(orderLineData.getSubReasonCode()));
				}
				if (null != orderLineData.getComments() && !orderLineData.getComments().isEmpty())
				{
					refundEntryModel.setReturnRemarks(orderLineData.getComments());
				}
				//TPR-5954 || End
				//refundEntryModel.setReason(RefundReason.valueOf(getReasonDesc(orderLineData.getReasonCode())));//TPR-5954
				refundEntryModel.setStatus(ReturnStatus.RETURN_INITIATED);
				refundEntryModel.setAction(ReturnAction.IMMEDIATE);
				//refundEntryModel.setNotes(getReasonDesc(orderLineData.getReasonCode()));TPR-5954
				refundEntryModel.setExpectedQuantity(abstractOrderEntryModel.getQuantity());//Single line quantity
				refundEntryModel.setReceivedQuantity(abstractOrderEntryModel.getQuantity());//Single line quantity
				refundEntryModel.setRefundedDate(new Date());
	...
	//TPR-5954 :: For fetching description from reason code
	@Override
	...
	private void constructQuickCilverOrderEntry(String transactionId, final OrderModel subOrderModel, final List<WalletCardApportionDetailModel> walletCardApportionDetailModelList,final OrderCancelRecordEntryModel orderRequestRecord){
