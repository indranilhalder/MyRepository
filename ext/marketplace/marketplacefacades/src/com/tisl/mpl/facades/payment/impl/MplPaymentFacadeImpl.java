...
public class MplPaymentFacadeImpl implements MplPaymentFacade
{
			...
			final QCRedeeptionResponse qcRedeeptionResponse, final double walletTotal, final double juspayTotalValue)
	{
		...
		for (final AbstractOrderEntryModel abstractOrderEntryModel : orderToBeUpdated.getEntries())
		{
			...
			//			{
			final WalletApportionPaymentInfoModel walletApportionPayment = getModelService().create(
					WalletApportionPaymentInfoModel.class);
			...
			{
				final WalletCardApportionDetailModel walletCardApportionObj = getModelService().create(
	...
	public mplNoCostEMIBankTenureDTO noCostEmiBankTenureList()
	{
		...
		try
		{
			...
			for (final Map.Entry<EMIBankModel, List<MplNoCostEMIVoucherModel>> resultMap : bankCoupondataSet.entrySet())
			{
				...
				listofbanks.setLogoUrl(bank.getLogoImage() != null ? bank.getLogoImage().getURL2() : "");
