...
public class MplPaymentFacadeImpl implements MplPaymentFacade
{
	...
	public String fetchPhoneNumber(final AbstractOrderModel abstractOrderModel) //Parameter AbstractOrderModel added extra for TPR-629
	{
		...
		if (null != abstractOrderModel)
		{
			if (null != abstractOrderModel.getDeliveryAddress() && null != abstractOrderModel.getDeliveryAddress().getPhone1())
			{
				...
				if (null != abstractOrderModel.getDeliveryAddress() && null != abstractOrderModel.getDeliveryAddress().getCellphone())
			{
			...
			final String walletOrderId)
	{
		...
		getMplPaymentService().entryInTPWaltAudit(status, mWRefCode, channelWeb, guid, walletOrderId);
	...
	public void saveTPWalletPaymentInfo(final AbstractOrderModel cart, final HttpServletRequest request)
	{
		...
		if (null != cart)
		{
			...
			getMplPaymentService().setTPWalletPaymentTransaction(paymentMode, cart, refernceCode);
			...
			if (null != mplCustomer)
			{
				if (StringUtils.isNotEmpty(mplCustomer.getName()) && !mplCustomer.getName().equalsIgnoreCase(" "))
				{
					...
					getMplPaymentService().saveTPWalletPaymentInfo(custName, entries, cart, refernceCode);
				}
				else
				{
					final String custEmail = mplCustomer.getOriginalUid();
					...
					getMplPaymentService().saveTPWalletPaymentInfo(custEmail, entries, cart, refernceCode);
				}
			}
			getMplPaymentService().paymentModeApportion(cart);
