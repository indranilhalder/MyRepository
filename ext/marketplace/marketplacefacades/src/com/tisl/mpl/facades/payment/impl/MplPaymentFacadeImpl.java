...
public class MplPaymentFacadeImpl implements MplPaymentFacade
{
	...
	public String validateOTPforCODWeb(final String customerID, final String enteredOTPNumber)
	{
		try
		{
			...
			final OTPResponseData otpResponse = getOtpGenericService().validateLatestOTP(customerID, null, enteredOTPNumber,
	...
	public String validateOTPforCODWV(final String customerID, final String enteredOTPNumber)
	{
		try
		{
			...
			final OTPResponseData otpResponse = getOtpGenericService().validateLatestOTPWV(customerID, null, enteredOTPNumber,
					OTPTypeEnum.COD, Long.parseLong(
							getConfigurationService().getConfiguration().getString(MarketplacecommerceservicesConstants.TIMEFOROTP)));
	...
	public void saveCODPaymentInfo(final Double cartValue, final Double totalCODCharge,
			final AbstractOrderModel abstractOrderModel) //Parameter abstractOrderModel added extra for TPR-629
		...
		try
		{
			...
			if (StringUtils.isNotEmpty(juspayOrderId)) //TISPT-200
			{
				...
				if (null != orderStatusResponse)
				{
					...
					if (/* orderModel.getTotalPriceWithConv().equals */(null != orderStatusResponse.getAmount()
	...
	{
		final String key = getConfigurationService().getConfiguration()
				.getString(MarketplacecommerceservicesConstants.JUSPAYMERCHANTTESTKEY);
			...
			final QCRedeeptionResponse qcRedeeptionResponse, final double walletTotal, final double juspayTotalValue)
	{
		...
		for (final AbstractOrderEntryModel abstractOrderEntryModel : orderToBeUpdated.getEntries())
		{
			...
			if (null != abstractOrderEntryModel.getCurrDelCharge() && abstractOrderEntryModel.getCurrDelCharge().doubleValue() > 0)
			{
	...
	public void cliqCashFailReturns(final String returnAmount, final String orderNumber)
	{
		...
		//		final DecimalFormat daecimalFormat = new DecimalFormat("#.00");
		//
		...
		//			qcCreditRequest.setAmount(daecimalFormat.format(returnAmount));
		//			qcCreditRequest.setInvoiceNumber(orderNumber);
		//			qcCreditRequest.setNotes("Cashback for " + daecimalFormat.format(returnAmount));
		//			final QCRedeeptionResponse response = mplWalletFacade.qcCredit(currentCustomer.getCustomerWalletDetail().getWalletId(),
		//					qcCreditRequest);
		//			LOG.debug("*****************************" + response.getResponseCode());
		//if(null != response && null != response.get)
		//			constructQuickCilverOrderEntry(response, response.getTransactionId().toString(), orderNumber);
		//			LOG.info("Refund Succesfull for Customer Wallet Id :" + currentCustomer.getCustomerWalletDetail().getWalletId()
		//					+ " Order Number :" + orderNumber);
		//		}
		//		else
		//		{
		//			LOG.error("Customer Wallet Incative can not Refund for order Number : " + orderNumber);
		//		}


	}

	//	private void constructQuickCilverOrderEntry(final QCRedeeptionResponse response, final String transactionId,
	...
	//
	//			walletApportionModel.setQcApportionPartValue(refundAmount);
	//
	//			if (StringUtils.equalsIgnoreCase(response.getResponseCode().toString(), "0"))
	//			{
	//				walletApportionModel.setType("REFUND");
	//			}
	//
	//			modelService.save(walletApportionModel);
	//			//walletApportionModel.setWalletApportionReturnInfo(walletApportionModel);
