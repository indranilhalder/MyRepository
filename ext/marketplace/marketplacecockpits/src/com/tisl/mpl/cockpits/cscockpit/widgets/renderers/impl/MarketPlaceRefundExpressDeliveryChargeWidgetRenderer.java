...
public class MarketPlaceRefundExpressDeliveryChargeWidgetRenderer extends
				...
				final NumberFormat currencyInstance = (NumberFormat) getSessionService()
						.executeInLocalView(new SessionExecutionBody() {
							public Object execute() {
								MarketPlaceRefundExpressDeliveryChargeWidgetRenderer.this
										.getCommonI18NService()
										.setCurrentCurrency(cartCurrencyModel);
								return MarketPlaceRefundExpressDeliveryChargeWidgetRenderer.this
										.getFormatFactory()
									...
									if (MapUtils.isNotEmpty(refundMap)) {
										...
										for (Map.Entry<AbstractOrderEntryModel, RefundDeliveryData> refundEntry : refundMap
											...
											if(null != refundEntry.getKey().getIsEDtoHD() && refundEntry.getKey().getIsEDtoHD() ) {
												Double hdDelCharges = refundEntry.getKey().getHdDeliveryCharge() != null ?refundEntry.getKey()
														.getHdDeliveryCharge() : NumberUtils.DOUBLE_ZERO;
												Double currDelCharges = 0.0D;
												if(null != refundEntry.getKey().getCurrDelCharge() && refundEntry.getKey().getCurrDelCharge() > 0.0D) {
													currDelCharges = refundEntry.getKey().getCurrDelCharge();
												}else if(null != refundEntry.getKey().getRefundedDeliveryChargeAmt() && refundEntry.getKey().getRefundedDeliveryChargeAmt() > 0.0D){
													currDelCharges = refundEntry.getKey().getRefundedDeliveryChargeAmt();
												}
												totalRefundDeliveryCharges = totalRefundDeliveryCharges
														+ (currDelCharges-hdDelCharges);
											}
		...
		if (CollectionUtils.isNotEmpty(orderEntryList)) {
			CollectionUtils.filter(orderEntryList, new Predicate() {
				...
				public boolean evaluate(Object o) {
					...
					if(null != entry.getIsEDtoHD() && entry.getIsEDtoHD() ) {
						boolean isEdAmountRefunded = false;
						if(null != entry.getRefundedEdChargeAmt() && entry.getRefundedEdChargeAmt() > 0.0D) {
							isEdAmountRefunded = true;
						}
						Double hdDelCharges = entry.getHdDeliveryCharge() != null ?entry
								.getHdDeliveryCharge() : NumberUtils.DOUBLE_ZERO;
						Double currDelCharges = 0.0D;
						if(null != entry.getCurrDelCharge() && entry.getCurrDelCharge() > 0.0D) {
							currDelCharges = entry.getCurrDelCharge();
						}else if(null != entry.getRefundedDeliveryChargeAmt() && entry.getRefundedDeliveryChargeAmt() > 0.0D){
							currDelCharges = entry.getRefundedDeliveryChargeAmt();
						}
						return	(currDelCharges > hdDelCharges) && !isEdAmountRefunded;
					}
					return false;
				}
			});
			...
			for (final AbstractOrderEntryModel orderEntry : orderEntryList) {
				if (orderEntry instanceof AbstractOrderEntryModel) {
					...
					Double hdDelCharges = orderEntry.getHdDeliveryCharge() != null ?orderEntry
							.getHdDeliveryCharge() : NumberUtils.DOUBLE_ZERO;
					Double currDelCharges = 0.0D;
					if(null != orderEntry.getCurrDelCharge() && orderEntry.getCurrDelCharge() > 0.0D) {
						currDelCharges = orderEntry.getCurrDelCharge();
					}else if(null != orderEntry.getRefundedDeliveryChargeAmt() && orderEntry.getRefundedDeliveryChargeAmt() > 0.0D){
						currDelCharges = orderEntry.getRefundedDeliveryChargeAmt();
					}
					deliveryChargesString = currencyInstance
							.format(currDelCharges - hdDelCharges);
