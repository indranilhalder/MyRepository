...
CheckoutCartWidgetRenderer {
	...
	protected void handleCartEntryDeliveryModeChangeEvent(ListboxWidget<CheckoutCartWidgetModel, CheckoutController> widget, Event event, TypedObject item, boolean initial)
	{
		...
		if (initial )
		{
			...
			if(MarketplacecommerceservicesConstants.FINEJEWELLERY.equalsIgnoreCase(((AbstractOrderEntryModel) item.getObject()).getProduct().getProductCategoryType()))
			{
				final List<JewelleryInformationModel> jewelleryInfo = getMplJewelleryService().getJewelleryInfoByUssid(((AbstractOrderEntryModel) item.getObject())
						.getSelectedUSSID());
				if(CollectionUtils.isNotEmpty(jewelleryInfo) && StringUtils.isNotEmpty(jewelleryInfo.get(0).getPCMUSSID()))
				{
					...
					deliveryModes = ((MarketplaceCheckoutController) widget
							.getWidgetController()).getDeliveryModesAndCost(MarketplaceCockpitsConstants.INR,
									ussid);
				}
				else
				{
					LOG.error("either JewelleryInformationModel or PCMUSSID is empty for variant : "+ ((AbstractOrderEntryModel) item.getObject())
							.getSelectedUSSID());
				}
			}
			else{
	...
	protected HtmlBasedComponent createCartEntryDeliveryModeBlock(
		...
		if(MarketplacecommerceservicesConstants.FINEJEWELLERY.equalsIgnoreCase(cartEntryModel.getProduct().getProductCategoryType()))
		{
			jewelleryInfo = getMplJewelleryService().getJewelleryInfoByUssid(cartEntryModel.getSelectedUSSID());
		}	
		...
		if(CollectionUtils.isNotEmpty(jewelleryInfo))
		{
			...
			{
				LOG.error("PCMUSSID is empty for variant : "+ cartEntryModel.getSelectedUSSID());
			}
		}
		else{
			deliveryModes = ((MarketplaceCheckoutController) widget
