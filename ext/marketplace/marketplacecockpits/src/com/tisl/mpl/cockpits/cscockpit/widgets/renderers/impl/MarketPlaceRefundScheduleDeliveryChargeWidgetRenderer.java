...
public class MarketPlaceRefundScheduleDeliveryChargeWidgetRenderer extends
		OrderDetailsOrderItemsWidgetRenderer {
	static Logger LOG = Logger
			.getLogger(MarketPlaceRefundScheduleDeliveryChargeWidgetRenderer.class.getName());
	...
	protected HtmlBasedComponent createContentInternal(
		...
		LOG.info("Refund Schedule Delivery Charges " );
				...
				final NumberFormat currencyInstance = (NumberFormat) getSessionService()
						.executeInLocalView(new SessionExecutionBody() {
							public Object execute() {
								MarketPlaceRefundScheduleDeliveryChargeWidgetRenderer.this
										.getCommonI18NService()
										.setCurrentCurrency(cartCurrencyModel);
								return MarketPlaceRefundScheduleDeliveryChargeWidgetRenderer.this
										.getFormatFactory()
								...
								public void onEvent(Event arg0)
									...
									if (MapUtils.isNotEmpty(refundMap)) {
										...
										Boolean isorderCOD = ((MarketPlaceOrderController) widget
												.getWidgetController())
												.isOrderCODforScheduleDeliveryCharges(
										...
										if (!flag) {
											...
											TypedObject refundDeliveryChargesRequest = ((MarketPlaceOrderController) widget
													.getWidgetController())
													.createRefundScheduleDeliveryChargesRequest(
															(OrderModel) orderTypedObject
																	.getObject(),
															refundMap);
	...
	private boolean createOrderDataTable(
				...
				if (orderEntry instanceof AbstractOrderEntryModel) {
					...
					if (CollectionUtils.isNotEmpty(columnConfigurations)) {
						for (ColumnGroupConfiguration col : columnConfigurations) {
							...
							headerLabel.setSclass("scheduleDeliveryChargeHeaderLabel");
							tableHeader.appendChild(headerLabel);
						}
					}
					Listheader headerLabel3 = new Listheader("Schedule Delivery Charges");
					headerLabel3.setTooltip("Schedule Delivery Charges");
					headerLabel3.setSclass("scheduleDeliveryChargeHeaderLabel");
					...
					headerLabel2.setSclass("scheduleDeliveryChargeHeaderLabel");
			...
			for (final AbstractOrderEntryModel orderEntry : orderEntryList) {
				...
				if (orderEntry instanceof AbstractOrderEntryModel && refundFlag) {
					...
					String deliveryChargesString = (null != orderEntry
							.getScheduledDeliveryCharge()) ? currencyInstance
							.format(orderEntry.getScheduledDeliveryCharge()) : "";
		...
		if (!resultFlag) {
			parentVbox.appendChild(new Label(
					"No Request Available for Refund Schedule Delivery Charges"));
		}
