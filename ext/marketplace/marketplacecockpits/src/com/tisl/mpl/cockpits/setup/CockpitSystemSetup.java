 ...
 *
 */
package com.tisl.mpl.cockpits.setup;
 ...
 */
@SystemSetup(extension = MarketplaceCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(MarketplaceCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(MarketplaceCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						MarketplaceCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/marketplacecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
