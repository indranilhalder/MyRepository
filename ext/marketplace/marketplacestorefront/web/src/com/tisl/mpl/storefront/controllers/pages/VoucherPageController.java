...
public class VoucherPageController extends AbstractSearchPageController
{
	...
	protected static final Logger LOG = Logger.getLogger(VoucherPageController.class);


	protected static final String COUPON_LISTING_CMS_PAGE_ID = "couponListing";
	protected static final String VOUCHER_ID_PATH_VARIABLE_PATTERN = "/{voucherCode:.*}";

	protected static final String CHANNEL = "web";

	@RequestMapping(value = VOUCHER_ID_PATH_VARIABLE_PATTERN, method = RequestMethod.GET)
	public String offer(@PathVariable("voucherCode") final String voucherCode,
					...
					throws UnsupportedEncodingException, CMSItemNotFoundException
	{
		...
		try
		{
			final VoucherSearchEvaluator couponSearch = new VoucherSearchEvaluator(voucherCode, XSSFilterUtil.filter(searchQuery),
					page, showMode, sortCode);
			couponSearch.doSearch();

			populateModel(model, couponSearch.getSearchPageData(), showMode);

			model.addAttribute("hideDepartments", Boolean.TRUE);
			model.addAttribute("otherProducts", true);

			model.addAttribute(WebConstants.BREADCRUMBS_KEY,
					searchBreadcrumbBuilder.getBreadcrumbs(null, couponSearch.getSearchPageData()));
			model.addAttribute(WebConstants.BREADCRUMBS_KEY,
					Collections.singletonList(new Breadcrumb("#", "Vouchers", LAST_LINK_CLASS)));
			model.addAttribute("voucher", voucherCode);

			storeCmsPageInModel(model, getContentPageForLabelOrId(COUPON_LISTING_CMS_PAGE_ID));
		}
	...
	protected class VoucherSearchEvaluator
	{
		...
		public VoucherSearchEvaluator(final String voucherCode, final String searchQuery, final int page, final ShowMode showMode,
		...
		public void doSearch()
		{
			...
			if (searchQueryData.getValue() == null)
			{
				...
				searchPageData = searchFacade.dropDownSearchForCoupon(searchState, voucherCode, pageableData);
			...
			else
			{
				...
				searchState.setQuery(searchQueryData);
				final PageableData pageableData = createPageableData(page, getSearchPageSize(), sortCode, showMode);
				searchPageData = searchFacade.dropDownSearchForCouponListing(searchState, voucherCode, pageableData);
