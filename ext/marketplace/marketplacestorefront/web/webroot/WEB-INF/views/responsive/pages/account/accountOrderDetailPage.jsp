	...
	<div class="account" id="anchorHead">
		...
		<div class="wrapper">
			...
			<div class="right-account">
					...
					<ul class="product-block order-details">
						<li class="header">
							...
							<ul>
								...
								<li><span>Total: </span> <!-- TISSIT-1773 --> <%-- <format:price	priceData="${subOrder.totalPrice}" /> --%>
							...
							<div class="totals" id="anchor">
								...
								<ul>
									<li><spring:theme code="text.account.order.subtotal" />
										<format:price priceData="${subOrder.subTotal}" /></li>
									...
									<c:if test="${subOrder.mplPaymentInfo.paymentOption eq 'COD'}">
										<li><spring:theme
												...
												priceData="${subOrder.convenienceChargeForCOD}" /></li>
									</c:if>
									<%-- <li><spring:theme text="Gift Wrap:" /><span><format:price
												priceData="${subOrder.deliveryCost}"
												displayFreeForZero="true" /></span></li> --%>
									<li class="grand-total"><spring:theme
											code="text.account.order.total.new" text="Total" /> <c:choose>
											<c:when test="${subOrder.net}">
												<span class="amt"><format:price
														priceData="${subOrder.totalPriceWithTax}" /></span>
											</c:when>
								...
								<c:when
									test="${(subOrder.status eq 'PAYMENT_PENDING' || subOrder.status eq 'PAYMENT_TIMEOUT' || subOrder.status eq 'PAYMENT_FAILED') && empty subOrder.mplPaymentInfo}">
								...
								else if (paymentInfo.getPaymentOption().equalsIgnoreCase(MarketplacecommerceservicesConstants.NETBANKING))
								{
									...
									<c:set var="creditCardLine2"
										value="${fn:trim(creditCardBillingAddress.line2)}" />
									<c:set var="creditCardLine3"
										value="${fn:trim(creditCardBillingAddress.line3)}" />
						...
						<c:forEach items="${subOrder.sellerOrderList}" var="sellerOrder"
							...
							<c:forEach items="${sellerOrder.entries}" var="entry"
								...
								<c:if
									test="${entry.mplDeliveryMode.code ne 'click-and-collect'}">
						...
						<c:if test="${flag || paymentError}">
							<li class="item delivered first" id="shipping-track-order">
								<div class="item-header">
									...
									<c:set var="subOrderLine2"
										value="${fn:trim(subOrder.deliveryAddress.line2)}" />
									<c:set var="subOrderLine3"
										value="${fn:trim(subOrder.deliveryAddress.line3)}" />
									<div class="col-md-8 col-sm-6">
										<address>
											...
											<c:if test="${not empty subOrder.deliveryAddress.landmark}">
												<span data-tribhuvan="landmark">
													${fn:escapeXml(subOrder.deliveryAddress.landmark)}</span>,
									</c:if>
											<br>
											<span data-tribhuvan="city">
												${fn:escapeXml(subOrder.deliveryAddress.town)}</span>,&nbsp;
											<c:if test="${not empty subOrder.deliveryAddress.state}">
												<span data-tribhuvan="state">${fn:escapeXml(subOrder.deliveryAddress.state)}</span>,&nbsp;
											</c:if>
											<span data-tribhuvan="pincode">${fn:escapeXml(subOrder.deliveryAddress.postalCode)}</span>&nbsp;<span
												data-tribhuvan="country">${fn:escapeXml(subOrder.deliveryAddress.country.isocode)}</span>
											<br> <span data-tribhuvan="mobileNo">91&nbsp;${fn:escapeXml(subOrder.deliveryAddress.phone)}</span>
											<br>
										</address>
									</div>
								</div> <!-- R2.3: START -->
								<div class="col-md-4 col-sm-6">
									<div class="editIconCSS">
										<c:if test="${addressChangeEligible eq true}">
											<a href="#" id="changeAddressLink">Edit / Change Address
											</a>
										</c:if>
									</div>
								</div>
								...
								<div class="itemBorder">&nbsp;</div> <!-- R2.3: END -->
						</c:if>
						<c:forEach items="${filterDeliveryMode}" var="deliveryType">
							...
							<c:if test="${paymentError }">
								...
								<%-- <c:forEach items="${subOrder.sellerOrderList}" var="sellerOrder"
								varStatus="status"> --%>
								<c:set var="sellerOrder" value="${subOrder}" />
								<!-- // need to set for payment pending failed or timeout  -->
								...
								<c:forEach items="${sellerOrder.entries}" var="entry"
									varStatus="entryStatus">
									<c:if
										test="${deliveryType eq entry.mplDeliveryMode.code && paymentError}">

										<c:if
											test="${entry.mplDeliveryMode.code eq 'click-and-collect' }">
											<c:if
												test="${storeId ne entry.deliveryPointOfService.address.id }">
												<c:set var="pos"
												...
												<li class="item delivered first" id="shipping-track-order">
													<div class="item-header">
														...
														<c:forEach items="${subOrder.entries}"
															var="parentRefEntry">
															<c:if
																test="${parentRefEntry.deliveryPointOfService.address.id eq pos.id 
		                                           and parentRefEntry.mplDeliveryMode.code eq 'click-and-collect'}">
																<c:set value="${cncQuantity+parentRefEntry.quantity}"
																	var="cncQuantity" />
															</c:if>
														</c:forEach>
														<c:if test="${not empty cncQuantity}">
															<h3>${cncQuantity}Product(s)-Collect</h3>
														</c:if>
														<p style="font-size: 12px; font-weight: 600;">Store
															Address:</p>
														<br> <br>
														<c:if
															test="${not empty entry.deliveryPointOfService.address}">
															<address
																style="line-height: 18px; font-size: 12px; padding-top: 5px;">
																<c:if
																	test="${not empty entry.deliveryPointOfService.displayName}"> ${fn:escapeXml(entry.deliveryPointOfService.displayName)}<br>
																</c:if>
																...
																<c:if test="${not empty pos.country.isocode}">${fn:escapeXml(pos.country.isocode)}<br>
																</c:if>
																<c:if test="${not empty pos.phone}">	+91&nbsp; ${fn:escapeXml(pos.phone)} <br>
																</c:if>
															</address>
														</c:if>
													</div>
										...
										<div class="item-fulfillment">
											...
											<c:if test="${not paymentError}">
												<p>
													<spring:message
														code="text.orderHistory.seller.order.number"></spring:message>
												...
												<c:when test="${not empty entry.selectedDeliverySlotDate}">
													<p>
														<span style="font-weight: bold">
															${entry.mplDeliveryMode.name} :</span> <span>${entry.selectedDeliverySlotDate}
															&nbsp;, ${entry.timeSlotFrom}-${entry.timeSlotTo}</span>
													</p>
												</c:when>
												<c:otherwise>
													<c:if test="${not empty entry.eddDateBetWeen}">
														<span style="font-weight: bold">
															${entry.mplDeliveryMode.name} :</span>  ${entry.eddDateBetWeen}  
                                                     </c:if>
												</c:otherwise>
											</c:choose>
											...
											<div id="pickNo" style="font-size: 12px; padding-top: 5px;">
												${sellerOrder.pickupPhoneNumber}<br>
											</div>
											&nbsp; &nbsp;
											<c:if
												test="${entry.mplDeliveryMode.code eq 'click-and-collect'}">
												...
												<c:if test="${button ne false}">
													<c:choose>
														<c:when test="${not empty entry.consignment.status}">
															...
															<c:forEach items="${subOrderStatus}"
												...
												<c:if test="${editButton eq 'enable' and button ne false}">
													<p style="margin-top: -8px;">${entry.mplDeliveryMode.name}
														:</p>
													<!-- <div id="pickName" 
													...
													<a type="button" id="pickName" class="pickupeditbtn"
														style="color: #000; padding-left: 10px;">${sellerOrder.pickupName}</a>
													<!--  </div> -->
				...
				<div class="col-md-5">
					<c:if
						test="${fn:containsIgnoreCase(entry.returnMethodType , 'SELF_COURIER')}">
						<c:if test="${entry.isRefundable eq false }">
							<c:if
								test="${entry.consignment.status.code eq 'RETURN_INITIATED'}">
								<div class="awsInnerClass">
									Please provide AWB number, Logistics partner and upload POD <a
										class="awbNumberLink" id="awbNumberLink">here</a>
								</div>
								<!-- TISRLUAT-50 -->
					...
					<div class="cancellation-request-block">
						...
						<form:form class="return-form"
							...
							<div class="return-container">
								<ul class="products">
									<c:forEach items="${cancelProductMap[entry.orderLineId]}"
										var="entryCancel">
										<li class="item look">
								...
								<form:hidden path="ticketTypeCode" class="ticketTypeCodeClass"
									value="C" />
								<form:hidden path="orderCode" id="orderCode"
									class="subOrderCodeClass" value="${sellerOrder.code}" />
								<form:hidden path="transactionId" class="transactionIdClass"
									value="${entry.transactionId}" />
								<form:hidden path="ussid" class="ussidClass"
									value="${ussidClass}" />

								<input type="hidden" class="entryNumberClass" id="entryNumber"
							...
							<div class="buttons">
								...
								<!-- TISPRDT - 995 -->
								<button type="button" class="light-red cancel-confirm-detail"
						...
						<form class="return-form">
							<div class="return-container container">
								...
								<div class="reason questions">
									<label id="reasonTitle">Reason for Cancellation:</label> <span
										id="reasonDesc"></span>
								</div>
							</div>
							<div class="buttons">
								<button type="button" class="light-red blue" id="returnReject"
									onclick="reloadOrderDetailPage()">Close</button>
			...
			<c:set value="${trackStatusMap[entry.orderLineId]}" var="trackStatus" />
			<c:set value="${trackStatusMap[dotCode]}" var="trackPaymentStatus" />

			<c:set value="${trackStatus['APPROVED']}" var="approvedStatus" />
			<c:set value="${trackStatus['PROCESSING']}" var="processingStatus" />
			...
			<c:forEach items="${shippingStatus}" var="productStatus"
				varStatus="loop">
				<c:if
					test="${(productStatus.responseCode eq 'DELIVERED') or (productStatus.responseCode eq 'ORDER_COLLECTED')}">
					<c:set var="productDelivered" value="1" />
				</c:if>
			</c:forEach>
			<div class="deliveryTrack status" id="tracker_${entry.transactionId}">
				<ul class="nav">
				...
				<ul class="progtrckr tabs">
					...
					<c:if test="${paymentError}">
						...
						<li class="progress progtrckr-done paymentStatus processing"
							orderlineid="${entry.orderLineId}" ordercode="${subOrder.code}">
							<span class="start"></span> <c:set value="0" var="dotCount" /> <!-- Show only first and last result to restrict in 2 dots-->
							<c:forEach items="${paymentStatus}" var="productPaymentStatus"
								varStatus="loop">

								<c:choose>
									<c:when
										test="${productPaymentStatus.isSelected eq true && productPaymentStatus.isEnabled eq true}">
										<span class="dot trackOrder_${productPaymentStatus.colorCode}"
											index="${loop.index}"> <img
								...
								<c:if test="${productPaymentStatus.isEnabled eq true}">
									<c:set var="displayMsgVar"
										value="${displayMsgVar}<p>${productPaymentStatus.shipmentStatus}</p>" />
									<c:forEach items="${productPaymentStatus.statusRecords}"
										var="statusRecord">
										<c:set var="displayMsgVar"
								...
								<c:set
									value="${ (productPaymentStatus.responseCode eq currentStatusMap[dotCode]) ? 'block' : 'none'}"
									var="showBlock" />

								<c:if test="${productPaymentStatus.isSelected eq true}">
									<div
										class="order message trackOrdermessage_${productPaymentStatus.colorCode} order-placed-arrow"
					...
					<li class="progress progtrckr-done orderStatus processing"
						orderlineid="${entry.orderLineId}" ordercode="${subOrder.code}">
						<span class="start"></span> <c:set value="0" var="dotCount" /> <!-- Show only first and last result to restrict in 2 dots-->
						<c:forEach items="${approvedStatus}" var="productStatus"
							varStatus="loop">

							<c:choose>
								<c:when
									test="${productStatus.isSelected eq true && productStatus.isEnabled eq true}">
									<span class="dot trackOrder_${productStatus.colorCode}"
										index="${loop.index}"> <img
										src="${commonResourcePath}/images/thin_top_arrow_222.png"
										class="dot-arrow">
									</span>
							...
							<c:if test="${productStatus.isSelected eq true}">
								<div
									class="order message trackOrdermessage_${productStatus.colorCode} order-placed-arrow"
									id="orderStatus${entry.orderLineId}_${loop.index}"
									style="display: ${showBlock};">${displayMsgVar}</div>
							</c:if>
							...
							<span class="dot inactive"></span>
						</c:forEach> <span class="end "></span> <span class="end "></span>
					</li>
					<!--End Approval Block -->
					...
					<c:if
						...
						<li class="progress progtrckr-done processingStatus processing"
							orderlineid="${entry.orderLineId}" ordercode="${subOrder.code}">
							<c:set value="${0}" var="dotCount" /> <c:forEach
								items="${processingStatus}" var="productStatus" varStatus="loop">
								...
								<c:if test="${loop.last}">
									...
									<c:if test="${productStatus.isSelected eq true}">
										<div
											class="order message trackOrdermessage_${productStatus.colorCode} processing-stage-arrow1"
					...
					<c:if test="${fn:length(cancelStatus) gt 0}">
						...
						<li class="progress progtrckr-done cancelStatus processing"
							orderlineid="${entry.orderLineId}" ordercode="${subOrder.code}"><c:set
								value="${0}" var="dotCount" /> <c:forEach
								items="${cancelStatus}" var="productStatus" varStatus="loop">
								...
								<c:if test="${productStatus.isSelected eq true}">
									<div
										class="cancellation message trackOrdermessage_${productStatus.colorCode}"
					...
					<c:if test="${fn:length(cancelStatus) eq 0}">
						<c:set var="displayMsgVar" value="" />
						<li class="progress progtrckr-done shippingStatus processing"
							orderlineid="${entry.orderLineId}" ordercode="${subOrder.code}">
							<!-- <span class="start"></span> --> <c:set value="${0}"
								...
								<c:if test="${productStatus.isSelected eq true}">
									<div
										class="order message trackOrdermessage_${productStatus.colorCode} shipping tracking-information"
										...
										<c:if
											...
											<c:choose>
												<c:when test="${not empty trackingurl[entry.orderLineId]}">
													<p>
														AWB No. <a href="${trackingurl[entry.orderLineId]}">${awbNum[entry.orderLineId]}</a>
												</c:when>
												<c:otherwise>
												...
												<div id="track-more-info">
													<p class="active">
														<span class="view-more-consignment"
															orderlineid="${entry.orderLineId}" index="${loop.index}"
															ordercode="${subOrder.code}"> View more </span>
													</p>
													<p>
														<span class="view-more-consignment"
															orderlineid="${entry.orderLineId}" index="${loop.index}"
															ordercode="${subOrder.code}">View less</span>
													</p>
												</div>
												<div
					...
					<c:if
						...
						<li class="progress progtrckr-done returnStatus processing"
							orderlineid="${entry.orderLineId}" ordercode="${subOrder.code}"><span
							class="start return-start"></span> <c:set value="${0}"
								...
								<c:if test="${productStatus.isSelected eq true}">
									<div
										class="return message trackOrdermessage_${productStatus.colorCode}"
										...
										<!-- TISEE-5433 -->
										<c:if
											test="${not empty returnLogistic[entry.orderLineId] and fn:toLowerCase(returnLogistic[entry.orderLineId]) ne 'null' and entry.mplDeliveryMode.code ne 'click-and-collect'
																	}">
											<p>Logistic : ${returnLogistic[entry.orderLineId]}</p>
										</c:if>
										<c:if
											test="${not empty returnAwbNum[entry.orderLineId] and fn:toLowerCase(returnAwbNum[entry.orderLineId]) ne 'null' and entry.mplDeliveryMode.code ne 'click-and-collect'
																 	}">
											<c:choose>
												<c:when test="${not empty trackingurl[entry.orderLineId]}">
													<p>
														AWB No. <a href="${trackingurl[entry.orderLineId]}">${returnAwbNum[entry.orderLineId]}</a>
												</c:when>
												<c:otherwise>
...
--%>
		...
		<c:forEach items="${subOrder.sellerOrderList}" var="sellerOrder"
			...
			<input type="hidden" id="subOrderCode" value="${sellerOrder.code}" />
			<input type="hidden" id="newCode" value="${subOrder.code}" />
			<c:forEach items="${sellerOrder.entries}" var="entry"
				varStatus="entryStatus">
				<c:if test="${deliveryType eq entry.mplDeliveryMode.code}">
					...
					<div class="item-fulfillment">
						...
						<div id="pickNo"
							style="font-size: 12px; padding-top: 5px; display: none;">
							${sellerOrder.pickupPhoneNumber}<br>
						</div>
						&nbsp; &nbsp;
						<c:if test="${entry.mplDeliveryMode.code eq 'click-and-collect'}">
									...
									<div class="row">
										...
										<div class="col-md-4" style="z-index: 99;">
											<div class="row mobileWidth" style="z-index: 99;">
												...
												<div class="col-md-7">
													<input id="pickMobileNo" class="pickMobileNo" type="Text"
														name="mobileNo" maxlength="10"
														value="${sellerOrder.getPickupPhoneNumber()}" />
													<div class="error_text pickupPersonMobileError" style=""></div>
					...
					<div class="order">
								...
								<c:if
									...
									<c:if test="${entry.isRefundable eq false }">
										<c:if
											test="${entry.consignment.status.code eq 'RETURN_INITIATED'}">
											<div class="awsInnerClass">
												Please provide AWB number, <br />Logistics partner and
												upload POD <a id="awbNumberLink" class="awbNumberLink">here</a>
											</div>
											<!-- TISRLUAT-50 -->
											<return:lpDetailsUploadPopup entry="${entry}" />
										</c:if>
						...
						<div class="deliveryTrack status suman"
							...
							<ul class="progtrckr tabs">
								...
								<li class="progress progtrckr-done orderStatus processing"
									orderlineid="${entry.orderLineId}" ordercode="${subOrder.code}">
									<span class="start"></span> <c:set value="0" var="dotCount" />
									<!-- Show only first and last result to restrict in 2 dots--> <c:forEach
										items="${approvedStatus}" var="productStatus" varStatus="loop">
								...
								<c:if
									...
									<li class="progress progtrckr-done processingStatus processing"
										...
										ordercode="${subOrder.code}"><c:set value="${0}"
											var="dotCount" /> <c:forEach items="${processingStatus}"
											var="productStatus" varStatus="loop">
...
--%>
	...
	<div class="removeModalAfterLoad" id="otpPopup"></div>
	</div>
	<!-- /.modal -->
...
@media ( max-width : 720px) {
	body .account .right-account .order-history.order-details li.item .item-header
		{
		margin-bottom: 0px
	}
	#changeAddressLink {
