...
<script>
	var imagePageLimit = ${count};	
	 $(document).ready(
			function() {
				var mainImageHeight = $(".main-image")
						.find("img.picZoomer-pic").height();
				var thumbnailImageHeight = (mainImageHeight / 5);
				var buttonHeight = $(".productImageGallery #previousImage")
						.outerHeight();
				...
				if ($("#previousImage").length) {
					...
					$(".productImageGallery").css("max-height",
							(mainImageHeight - buttonHeight) + "px");
				}
...
<div class="productImageGallery pdp-gallery image-list"
	style="position: relative;">
	<c:if test="${thumbNailImageLength > count}">

		<button type="button" class="previous white" style="margin: 0 auto;"
			name="previousImageDevBtn" id="previousImage"
			onclick="previousImage()">
			<img src="${commonResourcePath}/images/thin_top_arrow_333.png" />
			<%-- <spring:theme code="product.othersellers.previous" /> --%>
		</button>
	</c:if>
	<div class="imageList" style="overflow: hidden;">
		<ul class="jcarousel-skin imageListCarousel"
			style="display: none; position: relative; top: 0; width: 100%;">

			<c:forEach items="${galleryImages}" var="container"
				varStatus="varStatus" begin="0" end="${thumbNailImageLength}">
				<li id="addiImage${varStatus.index}"
					class="thumbailItem${varStatus.index +1}">
					<!-- For TPR-4687 --> <span class="thumb ${(varStatus.index==0)? "active":""}">
						<c:choose>
							...
							<c:otherwise>
								<c:if test="${container.thumbnail.mediaType.code eq 'Image'}">
									<img src="${container.thumbnail.url}" data-type="image"
	...
	<input type="hidden" id="totalAdditionalImage"
		name="totalAdditionalImage" value="${thumbNailImageLength}">

	<c:if test="${thumbNailImageLength > count}">
		<button type="button" class="next white" name="nextImageDevBtn"
			style="margin: 0 auto; bottom: 0;" id="nextImage"
			onclick="nextImage()">
