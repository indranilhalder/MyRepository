		...
		<c:when test="${not empty entries}">
						...
						<li>
							...
							<div class="product">
								...
								<h3 class="product-name">
									<a href="${entryProductUrl}">${entry.product.productTitle}</a>
								</h3>
							...
							<ul class="item-edit-details">
								<li><spring:theme code="cart.popup.quantity" />
									${entry.quantity}</li>
								<c:if test="${not empty entry.product.size}">
									<li><spring:theme code="cart.popup.size" />&nbsp;${entry.product.size}</li>
								</c:if>
								<c:if test="${entry.giveAway ne true}">
									<li><a href="#nogo" class="removeFromCart"
										data-entry-no="${entry.entryNumber}"
										data-ussid="${entry.selectedUssid}"><spring:theme
												code="text.iconCartRemove" /></a></li>
								</c:if>
							</ul>
						</li>
						<c:if test="${entry.giveAway ne true}">
							<%-- <li class="price"><format:price priceData="${entry.basePrice}"/></li> --%>
							<li class="price"><format:price
									priceData="${entry.totalPrice}" /></li>
			...
			<li><c:if test="${numberItemsInCart > numberShowing}">
					...
					<div class="more-items">${numberItemsInCart -numberShowing}
						...
						<c:if test="${numberItemsInCart -numberShowing > 1}">&nbsp;<spring:theme
								code="cart.popup.more.items" />
						</c:if>
						<c:if test="${numberItemsInCart -numberShowing == 1}">&nbsp;<spring:theme
								code="cart.popup.more.item" />
						</c:if>
					</div>
				</c:if> <%-- <div class="totalPayable">*** ${(cartData.deliveryCost.formattedValue)}
					...
					<c:when
						test="${not empty cartData.deliveryCost.formattedValue &&  cartData.deliveryCost.formattedValue gt  '0.0'}">
						<c:set var="res"
							value="${(cartData.totalPriceWithConvCharge.value - cartData.deliveryCost.value)}" />
						<div class="totalPayable">
							<spring:theme code="cart.popup.totalPayable" />
							<span class="total-payable-amount"> &#x20B9; <fmt:formatNumber
					...
					<c:otherwise>
						<div class="totalPayable">
							<spring:theme code="cart.popup.totalPayable" />
							<span class="total-payable-amount">${cartData.totalPriceWithConvCharge.formattedValue}</span>
						</div>
					</c:otherwise>
				</c:choose> <a href="${cartUrl}" class="go-to-bag mini-cart-checkout-button">
