...
<ul class="product-block">
	...
	<li class="header">
		<ul>
			...
			<li class="delivery"><spring:theme code="cart.delivery.options" /><span
				id="pincodeforcart"></span></li>
	...
	<c:forEach items="${cartData.entries}" var="entry" varStatus="status">
		...
		<div>
			...
			<input type="hidden" value="${entry.product.rootCategory}"
				name="rootCategoryMSD" /> <input type="hidden"
				name="productCodeMSD" class="cartMSD" value="${entry.product.code}" />
			<input type="hidden" name="basePriceForMSD" class="cartMSD"
				value="${entry.basePrice.formattedValue}" /> <input type="hidden"
				name="subPriceForMSD" class="cartMSD"
		...
		<li class="item" id="entry-${entry.entryNumber}">
			<ul class="desktop">
				...
				<li class="productItemInfo">
					...
					<div class="product-img">
						...
						<c:if
							test="${fn:toLowerCase(entry.product.luxIndicator)=='marketplace' or empty entry.product.luxIndicator and entry.product.rootCategory == 'FineJewellery'}">
							<a href="${productUrl}"><product:productPrimaryImage
									product="${entry.product}" format="fineJewelcartPage" /></a>
						</c:if>
					...
					</div> <span id="defaultWishId" style="display: none"><spring:theme
							code="wishlist.defaultname" /></span>
						...
						<ul class="item-edit-details">
							<c:if test="${entry.updateable}">
								...
								<ycommerce:testId code="cart_product_removeProduct">
									<li><a class="remove-entry-button"
										id="removeEntry_${entry.entryNumber}_${entry.selectedUssid}"><span><spring:theme
						...
						<ycommerce:testId code="cart_totalProductPrice_label">
							<c:choose>
								<c:when test="${entry.giveAway}">
									<!-- For Freebie item price will be shown as free -->
									...
									<c:choose>
										<c:when test="${entry.isBOGOapplied eq true}">
											<span class="delSeat" id="totalPrice_${entry.entryNumber}">
												<format:price priceData="${strikeoffprice}"
										...
										<c:when
											test="${entry.basePrice.formattedValue == entry.totalPrice.formattedValue}">
											<%-- TISPRO-215--%>
											<c:choose>
												<c:when
													test="${not empty entry.cartLevelDisc || not empty entry.productLevelDisc || not empty entry.productPerDiscDisplay}">
													<!-- TPR-970 changes-->
													<c:set var="totalPrice"
														value="${entry.totalPrice.formattedValue}" />
													<!-- TPR-970 changes-->
													<span class="delSeat" id="totalPrice_${entry.entryNumber}">
														<!-- TPR-970 changes--> <format:price
															priceData="${entry.totalMrp}" displayFreeForZero="false" />
													</span>
												</c:when>
												<c:otherwise>
													<%-- <c:choose> --%>
													...
													<!-- TPR-970 changes-->
													<c:set var="totalPrice"
														value="${entry.totalMrp.formattedValue}" />
													<!-- TPR-970 changes-->
													<span id="totalPrice_${entry.entryNumber}" class="delSeat"><format:price
															priceData="${entry.totalMrp}" /></span>
													<%-- <span><format:price priceData="${entry.totalMrp}"/></span> --%>
													<!-- </del> -->
												...
												<c:when
													...
													<c:choose>
														<c:when
															...
															<!-- TPR-970 changes-->
															<span id="totalPrice_${entry.entryNumber}"
																class="delSeat"><format:price
																	priceData="${entry.totalPrice}" /> <format:price
																	priceData="${entry.totalMrp}"
																	displayFreeForZero="false" /> </span>
														</c:when>
														<c:otherwise>
															<span id="totalPrice_${entry.entryNumber}"
																class="delSeat"><format:price
														...
														<c:forEach items="${mrpPriceMap}" var="mrpPrice">
															<c:choose>
																<c:when test="${mrpPrice.key == entry.entryNumber}">
																	<c:choose>
																		<c:when
																			test="${mrpPrice.value.formattedValue != entry.totalMrp.formattedValue||not empty entry.cartLevelDisc}">
																			<!-- TPR-970 changes-->
																			<input type="hidden"
																				id="basePrice_${entry.entryNumber}"
																				value='${mrpPrice.value.formattedValue}' />
																			<!-- TPR-970 changes-->
																			<li><span id="totalPrice_${entry.entryNumber}"
																				class="delSeat"> <format:price
																						priceData="${mrpPrice.value}"
																						displayFreeForZero="true" /></span></li>
																		</c:when>
																		<c:otherwise>
																			<c:if
																				test="${mrpPrice.value.formattedValue != entry.totalMrp }">
																				<!-- TPR-970 changes-->
																				<li><span id="totalPrice_${entry.entryNumber}"
																					class="delSeat"> <format:price
																							priceData="${mrpPrice.value}"
										...
										<c:when test="${not empty entry.productPerDiscDisplay}">
											<c:choose>
												<c:when test="${not empty entry.productLevelDisc}">
													<!-- TPR-970 changes-->
													<span id="itemCartCentDisplay_${entry.entryNumber}"><span
														class="off-bag">${entry.productPerDiscDisplay.value}<spring:theme
																code="off.item.percentage" /><span class="delSeat"><format:price
																	priceData="${entry.netSellingPrice}" /></span></span></span>
												</c:when>
												<c:otherwise>
													<!-- TPR-970 changes-->
													<span id="itemCartCentDisplay_${entry.entryNumber}"><span
														class="off-bag">${entry.productPerDiscDisplay.value}<spring:theme
																code="off.item.percentage" /><span class="delSeat"><format:price
																	priceData="${entry.totalPrice}" /></span></span></span>
												</c:otherwise>
											</c:choose>
										</c:when>
										<c:otherwise>
									...
									<c:choose>
										<c:when test="${not empty entry.productPerDiscDisplay}">
											<c:choose>
												<c:when test="${not empty entry.productLevelDisc}">
													<!-- TPR-970 changes-->
													<span id="itemCartAmtDisplay_${entry.entryNumber}"><span
														class="off-bag">${entry.productPerDiscDisplay.value}<spring:theme
																code="off.item.percentage" />
															<format:price priceData="${entry.netSellingPrice}" /></span></span>
												</c:when>
												<c:otherwise>
													<!-- TPR-970 changes-->
													<span id="itemCartAmtDisplay_${entry.entryNumber}"><span
														class="off-bag">${entry.productPerDiscDisplay.value}<spring:theme
																code="off.item.percentage" />
															<format:price priceData="${entry.totalPrice}" /></span></span>
												</c:otherwise>
											</c:choose>
										</c:when>
										<c:otherwise>
							...
							<c:if test="${not empty entry.cartLevelDisc}">
								<c:choose>
									...
									<c:otherwise>
										<c:if test="${entry.amountAfterAllDisc.value gt 0.1}">
											<span id="cartAmtOfferDisplay_${entry.entryNumber}"><span
												class="off-bag"><format:price
														priceData="${entry.cartLevelDisc}" />
													<spring:theme code="off.bag" />
													<format:price priceData="${entry.amountAfterAllDisc}" /></span></span>
							...
							<span id="ItemAmtofferDisplay_${entry.entryNumber}"
								style="display: none" class="ItemAmtofferDisplayPrFm"><span
								class="priceFormat priceFormatOnUpdate"><span
									id="off-bag-ItemLevelDisc_${entry.entryNumber}"></span></span><br />
							<span class="priceFormat"><span
									id="off-bag-ItemLevelDiscAmt_${entry.entryNumber}"></span></span></span>
							<span id="CartofferDisplay_${entry.entryNumber}"
								style="display: none" class="ItemAmtofferDisplayPrFm"><span
								class="priceFormat priceFormatOnUpdate"><span
									id="off-bag-cartLevelDisc_${entry.entryNumber}"></span></span><br />
							<span class="priceFormat"><span
									id="off-cartLevelDiscAmt_${entry.entryNumber}"></span></span></span>
				...
				<form:form id="${updateFormId}" action="${cartUpdateFormAction}"
						...
						<c:choose>
							<c:when test="${price lt 0.1 && entry.giveAway}">
								<form:select path="quantity"
									id="quantity_${entry.selectedUssid}_${entry.giveAway}"
		...
		<c:choose>
			<c:when test="${entry.giveAway}">
				...
				<li id="${entry.selectedUssid}_li_${entry.giveAway}"
			...
			<c:otherwise>
				<li id="${entry.selectedUssid}_li" class="delivery">
					...
					<p class="cartItemBlankPincode">
						<a href="#defaultPinCodeIds"><spring:theme
								code="cart.pincode.blanklink" />&nbsp;</a>
		...
		<c:choose>
			...
			<c:when
				test="${empty selectedPincode ||  fn:length(selectedPincode) == 0  }">
				<!-- TPR-1341 -->
...
<div class="add-to-wishlist-container">
	<form>
		...
		<div id="wishListDetailsId" class="other-sellers"
			...
			<button type='button'
				onclick="addToWishlistForCart($('#proUssid').val() , $('#selectedProductCode').val(),$('#alreadyAddedWlName_cart').val() )"
	...
	<div class="cart-total-block">
		...
		<ul class="totals">
			...
			<c:if test="${cartData.totalDiscounts.value > 0}">
				<li id="discount"><spring:theme
						code="basket.page.totals.savings" /><span class="amt"> <ycommerce:testId
							code="Order_Totals_Savings">
							<format:price priceData="${cartData.totalDiscounts}" />
						</ycommerce:testId> <spring:theme code="text.parenthesis.open" /> <c:out
							value="${cartData.discountPercentage}"></c:out>
		...
		<ul class="checkout-types onlyCheckoutButton checkOutBtnBtm">
			<li id="checkout-id-down" class="checkout-button"><a
				id="checkout-down-enabled"
				class="checkoutButton checkout button red"
				onclick="return checkServiceabilityRequired('typeCheckout',this);"><spring:theme
						code="checkout.checkout" /></a> <input type="hidden"
				id="checkoutLinkURlId" value="${checkoutUrl}">
				<p id="unserviceablepincode_tooltip_btm" style="display: none">
					One or more item(s) are not available at this location. Please
					remove the item(s) to proceed or try another <span>pincode</span>?
				</p>
				<p id="error-Id_tooltip_btm" style="display: none">
					Oops! Invalid <span>pincode</span>.Please enter a valid <span>pincode</span>.
				</p>
				<p id="emptyId_tooltip_btm" style="display: none">
					Enter <span>pincode</span> on the left to continue
				</p></li>
		</ul>
		...
		<ul class="cartBottomCheck">
			<li id="changePinDiv">
				<%-- <p><spring:theme code="product.pincode.input" /></p>
				<p id="cartPinCodeAvailableBtm" class="cartPins"><spring:theme code="product.pincode" /></p>  --%>
				<!-- TPR_1055 EQA --> <%-- <p id="AvailableMessageBtm" style="display:none"></p>
				<p id="unserviceablepincodeBtm" style="display:none"><spring:theme code="cart.unserviceable.pincode" /></p>
				<p id="error-IdBtm" style="display:none" ><spring:theme code="product.invalid.pincode" /></p>
				<p id="emptyIdBtm" style="display:none"><spring:theme code="product.empty.pincode" /></p> --%>
				<c:choose>
					<c:when test="${not empty defaultPinCode}">
						<input type="text" id="defaultPinCodeIdsBtm"
							name="defaultPinCodeIds" style="" value="${defaultPinCode}"
							placeholder="Pincode" maxlength="6"
							onkeypress="return isNumber(event)" />
					</c:when>
					<c:otherwise>
						<input type="text" id="defaultPinCodeIdsBtm"
							name="defaultPinCodeIds" style="" value="" placeholder="Pincode"
							maxlength="6" onkeypress="return isNumber(event)" />
					</c:otherwise>
				</c:choose>
				<button id="pinCodeButtonIdsBtm" name="pinCodeButtonId" style=""
					type=""
					onclick="return checkPincodeServiceability('typeSubmit',this);">
					<spring:theme code="text.submit" />
				</button>
				<p id="AvailableMessageBtm" style="display: none"
					class="availablePins"></p>
				<p id="unserviceablepincodeBtm" style="display: none"
					class="unservicePins">
					<spring:theme code="cart.unserviceable.pincode" />
				</p>
				<p id="error-IdBtm" style="display: none" class="errorPins">
					<spring:theme code="product.invalid.pincode" />
				</p>
				<p id="emptyIdBtm" style="display: none" class="emptyPins">
					<spring:theme code="product.empty.pincode" />
				</p>
			</li>
		</ul>
