...
<input type="hidden" id="page_subcategory_name"
	value="${page_subcategory_name}" />
<input type="hidden" id="page_subcategory_name_l3"
	value="${page_subcategory_name_l3}" />
...
<!-- CKD:TPR-250 -->
<input type="hidden" id="msiteBuyBoxSellerId"
	value="${msiteBuyBoxSellerId}" />
	...
	<div class="product-info wrapper">
		...
		<div class="product-detail ${product.rootCategory}">
			...
			<ycommerce:testId
				...
				<c:choose>
					<c:when
						test="${not empty clickableBrandname && not empty clickableBrandCode}">
						<h3 itemprop="brand" itemscope
							itemtype="http://schema.org/Organization" class="company">
							<span itemprop="name"><a
								href="/${clickableBrandname}/c-${clickableBrandCode}">${product.brand.brandname}</a></span>
						</h3>
					</c:when>
					<c:otherwise>
						<h3 itemprop="brand" itemscope
							itemtype="http://schema.org/Organization" class="company">
							<span itemprop="name">${product.brand.brandname}</span>
						</h3>
					</c:otherwise>
				</c:choose>
				<!-- CKD:TPR-250-End-->
				<%-- <a itemprop="url" href="${mainurl}"> --%>
				<!-- Commented as part of UF-181 -->
				<!-- For TPR-4358 -->
			...
			<div class="SoldWrap">
				...
				<div class="seller">
					<ycommerce:testId
						code="productDetails_productNamePrice_label_${product.code}">
						<span class="seller-details"><span>Sold by <span
								id="sellerNameId"></span> <product:sellerInfoDetailsSection /></span></span>
					</ycommerce:testId>
