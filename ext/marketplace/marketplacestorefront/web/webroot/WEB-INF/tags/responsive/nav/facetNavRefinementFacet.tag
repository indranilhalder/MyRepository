					...
					<c:forEach items="${facetData.topValues}" var="facetValue">
						...
						<c:when test="${(facetData.code eq 'colour'  || facetData.code  eq 'dialColourWatches')&& not empty facetValue.name}">
							...
							<c:choose>
								...
								<input type="button" value="" style="background:url('${commonResourcePath}/images/multi.jpg');border:1px solid rgb(204, 211, 217);height:36px;padding: 13px 17px; width:36px;background-size:100%;">  <!-- CHanges done for TISQAUATS-12 button replaced with submit -->
