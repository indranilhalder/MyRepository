...
<spring:eval
	expression="T(de.hybris.platform.util.Config).getParameter('mpl.jewellery.category')"
	var="lengthVariant" />
...
<c:choose>
	<c:when test="${lengthSize eq true }">
		<span id="selectSizeId" style="display: none; color: #ff1c47"><spring:theme
				code="variant.pleaseselectlength" /></span>
	</c:when>
	<c:otherwise>
		<span id="selectSizeId" style="display: none; color: #ff1c47"><spring:theme
				code="variant.pleaseselectsize" /></span>
	</c:otherwise>
</c:choose>
...
<c:url var="sizeGuideUrl"
	value="/p-sizeGuide?productCode=${product.code}&sizeSelected=${selectedSize}"
	scope="request"></c:url>
<!--CKD:TPR-250 Start -->
<c:choose>
...
<div class="color-swatch-container">
	...
	<c:if test="${colorPresent==true}">
		<c:if test="${multiColorFlag eq 'true'}">
			<!-- UF-432 -->
			<p>
				<spring:theme code="variant.color" />
			</p>
		</c:if>
	</c:if>
	<ul class="color-swatch" style="margin-bottom: 0px;">
		<c:choose>
			<c:when test="${not empty product.variantOptions}">
				...
				</c:forEach> --%> <!-- TISPRO-467 --> <c:forEach
									items="${variantOption.colourCode}" var="color">
									<c:if test="${multiColorFlag eq 'true'}">
										<!-- UF-432 -->
										<c:choose>
											<c:when
												test="${fn:startsWith(color, 'multi') && empty variantOption.image}">
											...
											<c:when test="${empty variantOption.image}">
												<span
													style="background-color: ${color};border: 1px solid rgb(204, 211, 217); width:50px; height:73px"
													title="${variantOption.colour}"></span>
											</c:when>
											<c:otherwise>
												<c:set var="imageData" value="${variantOption.image}" />
												<img src="${imageData.url}" title="${variantOption.colour}"
													alt="${styleValue}"
				...
				<c:choose>
					<c:when test="${not empty currentColor}">
						<c:if test="${currentColor eq color}">
							...
							<span class="capacity-box"> <a href="${variantUrl}"
								class="js-reference-item cboxElement"
								data-quickview-title="<spring:theme code="popup.quick.view.select"/>"
								title="${variantOption.capacity}"> <c:forEach var="capacity"
										items="${variantOption.capacity}">
										<c:if test="${variantOption.code eq product.code}">
											<c:set var="currentCapacity"
												value="${variantOption.capacity}" />
	...
	<c:if test="${showSizeGuideForFA eq true}">
		<c:choose>
			...
			<c:when test="${selectedSize==null && pdpSizeCounter != 1}">
				<input type="hidden" name="sizeSelected" id="sizeSelected"
					value="no" />
			</c:when>
			<c:otherwise>
				<input type="hidden" name="sizeSelected" id="sizeSelected"
		...
		<!-- Size guide Pop-up -->
		<input type="hidden" value="${selectedSize}" name="isSizeSelectedQV"
			id="isSizeSelectedQV">
		...
		<form:form action="/" method="get" id="variantForm"
			class="sizeVariantForm quickview-popup ${product.rootCategory}">
			...
			<p class="sizetext size">
					...
					<c:when
						...
						<c:choose>
							<c:when test="${true eq lengthSize}">
								<span><spring:theme code="product.variant.length"></spring:theme>
									<c:if test="${not empty productSizeType}">(${productSizeType})</c:if>
								</span>
							</c:when>
							<c:otherwise>
								<span> <spring:theme code="product.variant.size"></spring:theme>
									<c:if test="${not empty productSizeType}">(${productSizeType})</c:if>
								</span>
							</c:otherwise>
						</c:choose>
				...
				<c:if test="${not empty buyingGuide}">
					<a class="buying-guide buying-guide home-buying-guide"
						role="button" onclick="redirectURL(buyingGuideData);"> <spring:theme
			...
			<div class="select-size" style="width: 50%;">
				...
				<ul label="sizes" id="quickViewVariant">
					<li style="display: none;"><a href="#"
						class="js-reference-item cboxElement">select size</a></li>
					<c:choose>
						<c:when
							test="${product.rootCategory=='FineJewellery' || product.rootCategory=='FashionJewellery'}">
							...
							<c:if
								test="${fn:length(product.variantOptions) eq 1 || pdpSizeCounter eq 1}">
							...
							<c:forEach items="${product.variantOptions}" var="variantOption">
								<c:forEach var="entry" items="${variantOption.sizeLink}">
									...
									<c:choose>
										<c:when test="${(variantOption.code eq product.code)}">
											<c:choose>
												<c:when test="${selectedSize eq null}">
													<li ${selectedClass}><a
														href="${link}?selectedSize=true${msiteSellerForSize}"
														class="js-reference-item cboxElement">${entry.value}</a></li>
												</c:when>
												<c:otherwise>
													<li class="selected"><a
														href="${link}?selectedSize=true${msiteSellerForSize}"
										...
										<c:otherwise>
											<li ${selectedClass} data-vcode="${link}"><a
												href="${link}?selectedSize=true${msiteSellerForSize}"
												class="js-reference-item cboxElement">${entry.value}</a></li>
										</c:otherwise>
									</c:choose>
						...
						<c:when test="${product.rootCategory=='HomeFurnishing'}">
							<c:choose>
								<c:when test="${productSizeQuick!='No Size'}">
														...
														<c:when test="${(variantOption.code eq product.code)}">
															<c:choose>
																<c:when test="${selectedSize eq null}">
																	<!--CKD:TPR-250:  -->
																	<li ${selectedClass}><a
																		href="${variantUrl}?selectedSize=true${msiteSellerForSize}"
																		class="js-reference-item cboxElement">${entry.value}</a></li>
																</c:when>
																<c:otherwise>
																	<!--CKD:TPR-250:  -->
...
<input type="hidden" maxlength="10" size="1" id="sellerSelId"
