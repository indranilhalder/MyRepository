...
function distinctVal(b){for(var a=[],d=0,f=b.length;d<f;d++)-1==a.indexOf(b[d])&&a.push(b[d]);return a}
...
function getFilteredRecommendations(b,a,d,f,g){var h=[],k=-1,p=0;jQuery.each(a,function(){if(!(0===parseInt(this.discounted_price)&&0<f.length)){var a=!1,a=0<parseInt(this.discounted_price)?!0:!1,e=!1;0===d.length&&(e=!0);for(var r=0;r<d.length;r++){var q=d[r].split(" - ");-1<d[r].indexOf("And")&&(q=d[r].replace("And","-").split(" - "));var m=parseInt(q[0].substr(1)),q=parseInt(q[1].substr(1));isNaN(q)&&(q="100000000");if(m<=parseInt(this.price)&&parseInt(this.price)<=q){e=!0;break}}0<d.length&&0<
f.length?!0===e&&!0===a&&(0===p%20&&(k++,h[k]=""),h[k]+=makeProductHtml(b,this,g),p++):0<d.length?!0===e&&(0===p%20&&(k++,h[k]=""),h[k]+=makeProductHtml(b,this,g),p++):0<f.length?!0===a&&(0===p%20&&(k++,h[k]=""),h[k]+=makeProductHtml(b,this,g),p++):(0===p%20&&(k++,h[k]=""),h[k]+=makeProductHtml(b,this,g),p++)}});for(a=0;5>a;a++)a<=k?($("#iapage"+(a+1)).css("visibility","visible"),$("#iapage_next").css("visibility","visible"),$("#iapage"+(a+1)).removeClass("active"),$("#iapage1").addClass("active")):
...
function makeProductHtml(b,a,d){var f=a.site_uss_id,g="",h="",k="",p=!1;if(null!=iaResponseData&&void 0!=iaResponseData)for(var l in iaResponseData)if(f==l){if(iaResponseData.hasOwnProperty(l))for(l=iaResponseData[l],f=0;f<l.length;f++)h=l[0],g=l[1],k=l[2];p=!0;break}if(p&&"undefined"!==typeof a.image_url){void 0!=a.start_date&&(daysDif=compareDateWithToday(new Date(a.start_date)),8>daysDif&&0<=daysDif&&(is_new_product=!0));void 0!=a.colors&&("string"===typeof a.colors&&(a.colors=[a.colors]),jQuery.each(a.colors,
function(b,d){"Pewter"==d?a.colors[b]="#8E9294":"Peach"==d?a.colors[b]="#FFDAB9":"Multi"==d?a.colors[b]="/store/_ui/responsive/common/images/multi.jpg":"Metallic"==d&&(a.colors[b]="#37FDFC")}));d=a.url+"/p/"+a.site_product_id+"/?iaclick=true&req="+d;0<spid.length&&(d+="&rspid="+spid);var e="",e=null!=a.colors&&2>a.colors.length&&null!=a.sizes&&2>a.sizes.length&&"Electronics"==a.type?e+('<li onmouseover="showBoth(this)" onmouseout="hideBoth(this)" class="look slide product-tile '+b+'_list_elements productParentList" style="display: inline-block; position: relative;">'):
e+('<li onmouseover="showQuickview(this)" onmouseout="hideQuickView(this)" class="look slide product-tile '+b+'_list_elements productParentList" style="display: inline-block;position: relative;">'),e=e+('<a href="'+d+'" class="product-tile" style="position: relative;">'),e=-1<a.image_url.indexOf("/")?e+('<div class="image" style="position: relative; left: 0;"><img class="product-image" style="font-size: 16px;text-overflow: ellipsis;" src="'+a.image_url+'" alt="'+a.name+'"/>'):e+('<div class="image" style="position: relative; left: 0;"><img class="product-image" style="font-size: 16px;text-overflow: ellipsis;" src="/_ui/desktop/theme-blue/images/missing-product-300x300.jpg" alt="'+
...
":"+g.getMinutes()+":"+g.getSeconds()),e;if((""!=a.sizes||null!=a.sizes||"undefined"!=a.sizes)&&0<a.sizes.length){g=[];$.each(a.sizes,function(b,d){a.sizes[b]=d.toUpperCase()});for(l=0;l<allsizes.length;l++)-1<a.sizes.indexOf(allsizes[l])&&g.push(allsizes[l]);0<g.length?a.sizes=g:a.sizes.sort();e+='</div><span class="sizesAvailable">Size : <span class="size-col">['+a.sizes+"] </span></span>"}e+="</div></a>";e+='<p style="font-size: 12px;margin-top: 33px;color: rgb(255, 28, 71);" id="status'+a.site_product_id+
'"></p>';return e+="</li>"}}
...
function updatePage(b,a){var d="";if("undefined"!==typeof b.data&&null!==b.data)if(-1<jQuery.inArray(a,productWidget)){if(d=productWidgetElement[jQuery.inArray(a,productWidget)],document.getElementById(d)){var f=b.data.recommendations,g=!0,h=b.data.recommendations,k=[],p=-1,l;for(l in h){var e=h[l].site_uss_id;k[++p]=e}l=k.join(",");$.ajax({contentType:"application/json; charset=utf-8",url:ACC.config.encodedContextPath+"/p-getIAResponse",async:!1,data:"iaUssIds="+l,cache:!1,dataType:"json",success:function(a){iaResponseData=
...
'</span><ul id="ia_category_select" style="width: auto;">');h+='<li class="sort_li" id="name-asc">Name: A to Z</li><li class="sort_li" id="name-desc">Name: Z to A</li>';h+='<li class="sort_li" id="price-asc">Price: Low to High</li>';h+='<li class="sort_li" id="price-desc">Price: High to Low</li>';h+="</ul></div></div>";k='<div class="select-view "><div class="select-list"><span class="selected hotSelected">All Departments</span><ul id="ia_category_select" style="width: auto;">';for(l=0;l<r.length;l++)0==
l&&(k+='<li class="category_li" id="allCat">All Departments</li>'),k+='<li class="category_li" id="'+q[l]+'">'+r[l]+"</li>";k+="</ul></div></div>";if(g){c="search"===site_page_type&&"ia_products_search"===d?c+'<h2><span style="color: black !important;">Best Sellers</span>':"viewSellers"===site_page_type&&"ia_products"===d?c+'<h2><span style="color: black !important;">You May Also Need</span>':"productpage"===site_page_type&&"ia_products_complements"===d?c+'<h2><span style="color: black !important;">Things That Go With This</span>':
...
c+='<li><input type="checkbox" id="sale-filter"><label for="sale-filter">On Sale</label></li></ul></li></div></ul></div>',c+='<div class="right-block">',c+='<div><h1 class="ia_trending">'+productWidgetTitle[jQuery.inArray(a,productWidget)]+"</h1>",c+="</div>","search"===a&&(c+=h),c+=k;c+='<ul id="'+d+'_list" class="product-list" style="width: 100%; float: left;margin-top: 55px; ">'}var m=[],w=-1,t="iapage1";if(g)jQuery.each(f,function(a,e){c+=makeProductHtml(d,this,b.request_id);recIndex++}),"hot"===
a&&"homepage"==site_page_type?(c+="</ul></div>",c+='</div></div><a href="http://'+window.location.host+'/viewAllTrending" class="button hotShowHide" style="display: inline-block;font-size: 12px;height: 42px;line-height: 42px;">Shop the Hot List</a>'):(c+="</ul></div>",c+="</div></div>");else{jQuery.each(f,function(a,c){0===recIndex%20&&(w++,m[w]="");m[w]+=makeProductHtml(d,this,b.request_id);recIndex++});c+=m[0];c+="</ul>";c+='<ul id="'+d+'page_numbers" class="pagination" style="position: absolute; right: 0;line-height: 0px;cursor: pointer;margin-top: 15px;padding:0px;">';
c+='<li id="iapage1" class="number first active iapage" style="padding: 5px;"><a>1</a></li>';for(l=1;l<m.length;l++)c+='<li id="iapage'+(l+1)+'" class="number iapage" style="padding: 5px;"><a>'+(l+1)+"</a></li>";c+='<li id="iapage_next" class="next" style="padding: 5px;"><a>Next <span class="lookbook-only"></span></a></li>';c+="</ul>";c+="</div>"}0<recIndex&&(document.getElementById(d).innerHTML=c);if("hot"===a||"search"===a)jQuery(".category_li").on("click",function(){var a=jQuery(this).text();$(".hotSelected").text(a);
...
sort_key?f.sort(iaSortNameDesc):"price-asc"===a?f.sort(iaSortPriceAsc):f.sort(iaSortPriceDesc);if(g)document.getElementById(d+"_list").innerHTML=reorderRecommendation(d,f,b.request_id);else{m=reorderRecommendationGrid(d,f,b.request_id);document.getElementById(d+"_list").innerHTML=m[0];for(var a=document.getElementById("ia_products_hotpage_numbers").childNodes,c=m.length;c<a.length-1;c++)a[c].visibility="visible";for(c=m.length;c<a.length-1;c++)a[c].visibility="hidden";t="iapage1"}});if(g)"search"==
site_page_type&&"ia_products_search"==d&&$("#"+d+"_list").owlCarousel({items:4,loop:!0,nav:!0,dots:!1,navText:[],responsive:{0:{items:1,stagePadding:50},480:{items:2,stagePadding:50},768:{items:3},1280:{items:5}}}),$("#"+d+"_list").owlCarousel({items:5,loop:!0,nav:!0,dots:!1,navText:[],responsive:{0:{items:1,stagePadding:50},480:{items:2,stagePadding:50},768:{items:3},1280:{items:5}}});else{jQuery(".iapage").on("click",function(){void 0!=m[parseInt(jQuery(this).text())-1]?document.getElementById(d+
"_list").innerHTML=m[parseInt(jQuery(this).text())-1]:document.getElementById(d+"_list").innerHTML="<h1 style='text-align:center;color: #22bfe6;font-size: 24px;'>No Results Found.</h1>";jQuery("#"+t).removeClass("active");jQuery(this).addClass("active");t=this.id});jQuery("#iapage_next").on("click",function(){var a=parseInt(t.replace(/^\D+/g,""))+1;5<a&&(a%=5);null!==document.getElementById("iapage"+a)&&(a=document.getElementById("iapage"+a),void 0!=m[parseInt(jQuery(a).text())-1]?document.getElementById(d+
"_list").innerHTML=m[parseInt(jQuery(a).text())-1]:document.getElementById(d+"_list").innerHTML="<h1 style='text-align:center;color: #22bfe6;font-size: 24px;'>No Results Found.</h1>",jQuery("#"+t).removeClass("active"),jQuery(document.getElementById(a.id)).addClass("active"),t=a.id)});var u=[],v=[];jQuery(".price li label").on("click",function(){-1<u.indexOf(this.textContent)?u.splice(u.indexOf(this.textContent),1):u.push(this.textContent);m=getFilteredRecommendations(d,f,u,v,b.request_id);void 0!=
m[0]||null!=m[0]?document.getElementById(d+"_list").innerHTML=m[0]:document.getElementById(d+"_list").innerHTML="<h1 style='text-align:center;color: #22bfe6;font-size: 24px;'>No Results Found.</h1>";for(var a=document.getElementById("ia_products_hotpage_numbers").childNodes,c=m.length;c<a.length-1;c++)a[c].visibility="visible";for(c=m.length;c<a.length-1;c++)a[c].visibility="hidden";t="iapage1"});jQuery(".on-sale li label").on("click",function(){-1<v.indexOf(this.textContent)?v.splice(v.indexOf(this.textContent),
1):v.push(this.textContent);m=getFilteredRecommendations(d,f,u,v,b.request_id);void 0!=m[0]?document.getElementById(d+"_list").innerHTML=m[0]:document.getElementById(d+"_list").innerHTML="<h1 style='text-align:center;color: #22bfe6;font-size: 24px;'>No Results Found.</h1>";for(var a=document.getElementById("ia_products_hotpage_numbers").childNodes,c=m.length;c<a.length-1;c++)a[c].visibility="visible";for(c=m.length;c<a.length-1;c++)a[c].visibility="hidden";t="iapage1"})}}}}else if(-1<jQuery.inArray(a,
