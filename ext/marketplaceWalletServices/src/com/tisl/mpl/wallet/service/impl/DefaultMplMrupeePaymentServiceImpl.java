 ...
 *
 */
public class DefaultMplMrupeePaymentServiceImpl implements DefaultMplMrupeePaymentService
{
	private static final Logger LOG = Logger.getLogger(DefaultMplMrupeePaymentServiceImpl.class);
	...
	{
		final String merchantCode = getConfigurationService().getConfiguration()
				.getString(MarketplaceWalletServicesConstants.MRUPEE_MERCHANT_CODE);

		final String narration = getConfigurationService().getConfiguration()
				.getString(MarketplaceWalletServicesConstants.MRUPEE_NARRATION);

		final String[] parameters =
		{ merchantCode, "P", walletOrderId, narration, amount };//Purchase
		final String checksumKey = getConfigurationService().getConfiguration()
				.getString(MarketplaceWalletServicesConstants.MRUPEE_CHECKSUM);
	...
	{
		final String merchantCodeVerfication = getConfigurationService().getConfiguration()
				.getString(MarketplaceWalletServicesConstants.MRUPEE_MERCHANT_CODE);
		final String[] parameters =
	...
	{
		final String merchantCodeRefund = getConfigurationService().getConfiguration()
				.getString(MarketplaceWalletServicesConstants.MRUPEE_MERCHANT_CODE);
