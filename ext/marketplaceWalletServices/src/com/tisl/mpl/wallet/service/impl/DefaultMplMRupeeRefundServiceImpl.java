 ...
 *
 */
package com.tisl.mpl.wallet.service.impl;
...
import com.tisl.mpl.wallet.service.DefaultMplMRupeeRefundService;
 ...
 *
 */
public class DefaultMplMRupeeRefundServiceImpl implements DefaultMplMRupeeRefundService
{

	private static final Logger LOG = Logger.getLogger(DefaultMplMRupeeRefundServiceImpl.class);
	...
	public MRupeeRefundResponse refund(final MRupeeRefundRequest refundRequest)
	{
		...
		//	final MrupeePaymentServiceImpl mRupeePaymentService = new MrupeePaymentServiceImpl();
		final String checksum = mRupeeService.generateCheckSum(refundRequest);
		...
		LOG.debug("MRUPEE serializedParams --------------serializedParams-----------request----" + serializedParams);

		final String mRupeeRequestParams = rmvSpecialCharfromCheksum(serializedParams);

		LOG.debug("Mrupee REFUND REQUEST--------------mRupeeRequestParams-----------request----" + mRupeeRequestParams);

		final String url = getConfigurationService().getConfiguration()
				.getString(MarketplaceWalletServicesConstants.MRUPEERETURNURL);

		LOG.debug("Request url --------------refundUrl-----------request----" + url);
		...
		final String response = makeServiceCall(url, mRupeeRequestParams);
	...
	/**
	 * For removing last & from the request parameter of the url
	 *
	 * @param serializedParams
	 * @return String
	 */
	public String rmvSpecialCharfromCheksum(final String serializedParams)
	{
		if (serializedParams.charAt(serializedParams.length() - 1) == '&')
		{
			String serializedParams1 = null;
			serializedParams1 = serializedParams.substring(0, serializedParams.length() - 1);
			return serializedParams1;
		}
		else
		{
	...
	public DefaultMplMRupeeRefundServiceImpl()
	{
