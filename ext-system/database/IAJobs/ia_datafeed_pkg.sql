...
AS
 ...
 PROCEDURE ia_brands
   IS
      ...
      v_brdolimg                BRDOLRR;
      --mobile image url cursor
      CURSOR BRDMBLIMG
      IS
         SELECT C.p_code, p_internalurl AS mobileurl
           FROM cat2medrel CM, medias M, categories C
          WHERE     C.PK = CM.SOURCEPK
                AND M.p_mediaformat = v_mobile_imgurl
                AND M.PK = CM.TARGETPK
                AND C.P_CATALOGVERSION = v_catalogversion_online
                AND c.p_code LIKE 'MBH%';
      TYPE BRDMBL IS RECORD
      (
         brandid   CATEGORIES.p_code%TYPE,
         mblurl    MEDIAS.p_internalurl%TYPE
      );
      TYPE BRDMBLLOGO IS TABLE OF BRDMBL;
      v_brdmblimg               BRDMBLLOGO;
   BEGIN
      -- assign the last run time value into local variable
      SELECT MPLBUYBOXUPDTLOG.P_LAST_RUN_TIME
     ...
     **/
      ...
      CLOSE BRDOVERLAYIMG;
      OPEN BRDMBLIMG;                                      /** new changes **/
      FETCH BRDMBLIMG BULK COLLECT INTO v_brdmblimg;
      FORALL i IN 1 .. v_brdmblimg.COUNT
         UPDATE DATAFEED.brands
            SET MOBILE_LOGO_IMAGE = v_brdmblimg (i).mblurl
          WHERE brand_id = v_brdmblimg (i).brandid;
      CLOSE BRDMBLIMG;
      --to Update the last run time
      UPDATE MPLBUYBOXUPDTLOG
