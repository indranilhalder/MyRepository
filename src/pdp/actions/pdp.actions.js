...
export function followUnFollowBrand(brandCode) {
  return async (dispatch, getState, { api }) => {
    ...
    try {
      let currentFollowedStatus = getState().productDescription.productDetails
        .isFollowing;
      let ifFollowing = !currentFollowedStatus;
      // getting market cloud id it is auto generated by use when use visti web MSSiteModeEvent
      // we just need to get it here
      let mcvId = window._satellite.getVisitorId().getMarketingCloudVisitorID();
      const result = await api.post(
        `${FOLLOW_UN_FOLLOW_PARK}/${mcvId}/updateFollowedBrands?brands=${brandCode}&follow=${ifFollowing}&isPwa=true`
      );
      const resultJson = await result.json();
      if (resultJson.status === FAILURE) {
        throw new Error(`${resultJson.message}`);
      }
      dispatch(followUnFollowBrandSuccess(ifFollowing));
    } catch (e) {
