...
export function followUnFollowBrand(brandCode) {
  return async (dispatch, getState, { api }) => {
    ...
    try {
      let currentFollowedStatus = getState().productDescription.msdItems
        .brandDetails.isFollowing;
      let isFollowing = !currentFollowedStatus;
      ...
      const result = await api.post(
        `${FOLLOW_UN_FOLLOW_PARK}/${mcvId}/updateFollowedBrands?brands=${brandCode}&follow=${isFollowing}&isPwa=true`
      );
      const resultJson = await result.json();
      if (resultJson.status === FAILURE) {
        throw new Error(`${resultJson.message}`);
